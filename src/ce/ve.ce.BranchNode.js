...
ve.ce.BranchNode.prototype.getDomPosition = function ( offset ) {
	...
	while ( true ) {
		...
		if ( !ceNode ) {
			// No children with DOM nodes; return the first offset inside domNode
			domOffset = 0;
			break;
		}
		if ( ceNode.$element && ceNode.$element.length > 0 ) {
			domOffset = Array.prototype.indexOf.call( domNode.childNodes, ceNode.$element.last()[ 0 ] ) + 1;
			break;
		}
		if ( ceNode.getType() === 'text' ) {
			// Text nodes do not have a reference to their DOM nodes. Find the
			// DOM node for an earlier node, then use its nextSibling later.
			textNodeCount++;
		}
		// Else no DOM representation (e.g. a MetaItem); move to previous element
	}
	if ( textNodeCount > 1 ) {
		// Note that CE text nodes can contain varying annotations and therefore
		// their DOM structure can be quite complex. We have skipped over
		// textNodeCount - 1 > 0 such nodes, so our calculation is unreliable
