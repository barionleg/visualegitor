...
ve.ce.Document.prototype.getNodeAndOffset = function ( offset ) {
	...
	/**
	 * Get the DOM leaf between the current and adjacent cursor positions
	 *
	 * @param {number} direction 1 for forwards, -1 for backwards
	 * @param {Node} node The current node
	 * @param {number} [offset] The current offset; defaults to the node's leading edge
	...
	function getAdjacentLeaf( direction, node, offset ) {
		...
		if ( node.nodeType === Node.TEXT_NODE && interior ) {
			// There is only text at the preceding cursor position
			return null;
		}
