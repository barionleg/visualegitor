...
ve.ce.Document.prototype.getNodeAndOffsetUnadjustedForUnicorn = function ( offset ) {
	...
	// - is in a text node: return that node and the correct remainder offset
	// - is between an empty unicorn pair: return inter-unicorn location
	// - is in a non-branchNode/leafNode DOM element: return first maximally deep such element
	// Otherwise, signal an error

	// Unfortunately, there is no way to avoid slugless block nodes with no DM length:
	// an IME can remove all the text from a node at a time when it is unsafe to fixup the
	// node contents. In such cases, a maximally deep element gives better bounding
	// rectangle coordinates than any of its containers.
