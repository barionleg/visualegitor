...
ve.ce.Document.prototype.getNodeAndOffset = function ( offset ) {
	var branchNode, position, count, step, node, model,
		countedNodes = [],
		found = {};

	// 1. Step with ve.adjacentDomPosition( ..., { skipSoft: false } ) until we hit a
	// position at the correct offset (which is guaranteed to be the first such position in
	// document order).
	// 2. Use ve.adjacentDomPosition( ..., { skipSoft: true } ) once to return all
	...
	ve.adjacentDomPosition(
		...
		found.text = found.text && ( step.stepType === Node.TEXT && step );
	} );
	found.unicorn = found.preUnicorn || found.postUnicorn;
	found.nail = found.preOpenNail || found.postOpenNail || found.preCloseNail || found.postCloseNail;
	...
	if ( found.text ) {
		...
		// We must have entered the text node start, so return that position
		return { node: found.text.node, position: 0 };
	}
	return position;
};
