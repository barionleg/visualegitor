...
ve.ce.ContentBranchNode.prototype.getRenderedContents = function () {
	...
	// TODO: Is normalize guaranteed to be a no-op?
	wrapper.normalize();
	wrapper.unicornInfo = unicornInfo;
	return wrapper;
};
...
ve.ce.ContentBranchNode.prototype.renderContents = function () {
	...
	const modelRenderedContents = this.getRenderedContents();
	...
	if ( this.rendered ) {
		const viewRenderedContents = ve.ce.ContentBranchNode.static.cloneForComparison(
			this.$element[ 0 ]
		);
	...
	const unicornInfo = modelRenderedContents.unicornInfo;
