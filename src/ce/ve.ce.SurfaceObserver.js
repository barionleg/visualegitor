 ...
 * @param {boolean} selectionOnly If false, the caller promises the content has not changed
 ...
 */
ve.ce.SurfaceObserver.static.getUpdatedRangeState = function ( oldState, docNode, selectionOnly ) {
	var $nodeOrSlug, selection, isAnchorNodeChanged,
		newState = {};
	...
	isAnchorNodeChanged = !oldState || oldState.isAnchorNodeChanged( selection );
	...
	newState.isDomRangeChanged = function ( futureSelection ) {
		// The meaning of isDomRangeChanged is slightly odd, because the offsets were taken
		// at two different instants, between which content outside of the selection may
		// have changed. This can in theory cause false negatives.
		return (
