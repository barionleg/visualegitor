...
ve.ce.SurfaceObserver.prototype.pollOnceInternal = function ( emitChanges, selectionOnly ) {
	...
	if ( newState.domRangeChanged && emitChanges ) {
		this.emit( 'rangeChange', oldState.range, newState.range );
	}
...
ve.ce.SurfaceObserver.static.getUpdatedRangeState = function ( oldState, docNode ) {
	var $nodeOrSlug,
		selection = docNode.getElementDocument().getSelection,
		newState = {};
	...
	if ( newState.node === null ) {
		...
		newState.text = ve.ce.getDomText( this.node.$element[0] );
		newState.hash = ve.ce.getDomHash( this.node.$element[0] );
	}
