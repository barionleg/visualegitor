...
ve.ce.FocusableNode.prototype.calculateHighlights = function () {
	...
	// As elements with an actual width/height of 1 aren't that useful anyyway, just
	// throw away anything that is <=1.
	filteredRects = rects.filter( function ( rect ) {
		return rect.width > 1 && rect.height > 1;
	} );
	// ...except if this doesn't leave any rects at all, then we do want to use the 1px rects
