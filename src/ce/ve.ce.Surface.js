...
ve.ce.Surface.prototype.getSelectionStartAndEndRects = function ( range ) {
	var focusedNode, selection = this.getModel().getSelection();
...
ve.ce.Surface.prototype.onDocumentBlur = function () {
	...
	this.dragging = false;
	this.focused = false;
	this.getModel().setEmptySelection();
	this.emit( 'blur' );
};
 ...
 *
 * Limits native drag and drop behavio
 *
 * @method
...
ve.ce.Surface.prototype.beforePaste = function ( e ) {
	...
	if ( !range.isCollapsed() ) {
		...
		this.model.change( tx, new ve.dm.LinearSelection( doc, range ) );
...
ve.ce.Surface.prototype.afterPaste = function () {
	...
	range = tx.translateRange( range );
	this.model.change( tx, new ve.dm.LinearSelection( this.getModel().getDocument(), new ve.Range( range.start ) ) );
	// Move cursor to end of selection
	this.model.setLinearSelection( new ve.Range( range.end ) );
};
	...
	try {
		this.changeModel(
			...
			newRange ?
				...
				new ve.dm.EmptySelection( this.getModel().getDocument() )
		);
	} finally {
		this.decRenderLock();
	}
