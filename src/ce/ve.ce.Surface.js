...
ve.ce.Surface.prototype.selectRelativeSelectableContentOffset = function ( startOffset, direction ) {
	...
	var nextOffset = linearData.getRelativeOffset(
		...
		function ( offset ) {
			...
			var activeNode = branchNode.traverseUpstream( function ( node ) {
				return !node.canBeActive();
			} );
			if ( activeNode && activeNode !== surface.getActiveNode() ) {
				// Don't try to place the cursor in an active node, unless it is active
				return false;
			}
			return true;
		}
