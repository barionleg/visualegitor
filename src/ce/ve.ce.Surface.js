	...
	try {
		...
		if ( rects.length === 0 ) {
			...
			if ( boundingRect.width === 0 && boundingRect.height === 0 ) {
				...
				// When nativeRange is a collapsed cursor at the end of a line or the start
				// of a line, the bounding rect is [0,0,0,0] in Chrome. getClientRects returns
				// two rects, one correct, and one at the end of the next line / start of the
				// previous line. We can't tell here which one to use to just pick the first.
				// This matches Firefox's behaviour, which tells you the cursor is at the end
