...
ve.ce.Surface.prototype.drawSelections = function ( name, selections, options ) {
	...
	selections.forEach( function ( selection ) {
		var $selection = surface.getDrawnSelection( name, selection.getModel(), options.color );
		if ( !$selection ) {
		...
		var cacheKey = surface.storeDrawnSelection( $selection, name, selection.getModel(), options.color );
	...
	oldSelections.forEach( function ( oldSelection ) {
		var cacheKey = surface.getDrawnSelectionCacheKey( name, oldSelection.getModel(), options.color );
		if ( !selectionsJustShown[ cacheKey ] ) {
			var $oldSelection = surface.getDrawnSelection( name, oldSelection.getModel(), options.color );
			if ( $oldSelection ) {
				$oldSelection.detach();
			}
		}
 ...
 * @param {string} [color] Inline color of selection
 * @return {string} Cache key
 */
ve.ce.Surface.prototype.getDrawnSelectionCacheKey = function ( name, selection, color ) {
	return name + '-' + JSON.stringify( selection ) + '-' + ( color || '' );
};
 ...
 */
ve.ce.Surface.prototype.getDrawnSelection = function ( name, selection, color ) {
	var cacheKey = this.getDrawnSelectionCacheKey( name, selection, color );
 ...
 * @return {string} Cache key
 */
ve.ce.Surface.prototype.storeDrawnSelection = function ( $selection, name, selection, color ) {
	var cacheKey = this.getDrawnSelectionCacheKey( name, selection, color );
