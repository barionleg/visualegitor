...
ve.ce.Surface.prototype.afterPaste = function () {
	...
	// HACK: Force non-async promises to resolve synchronously
	// to avoid test failures. TODO: Make tests async.
	if ( targetFragment.getPending().state() === 'resolved' ) {
		afterAfterPaste();
	} else {
		targetFragment.getPending().then( afterAfterPaste );
	}
};
