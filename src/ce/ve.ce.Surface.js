...
ve.ce.Surface.prototype.drawSelections = function ( name, selections, options ) {
	...
	selections.forEach( function ( selection ) {
		...
		if ( !$selection ) {
			...
			if ( options.label ) {
				$selection.children().first().append(
					$( '<span>' )
	...
	oldSelections.forEach( function ( oldSelection ) {
		var cacheKey = surface.getDrawnSelectionCacheKey( name, oldSelection.getModel(), options );
		if ( !selectionsJustShown[ cacheKey ] ) {
			var $oldSelection = surface.getDrawnSelection( name, oldSelection.getModel(), options );
			if ( $oldSelection ) {
				$oldSelection.detach();
			}
		}
...
ve.ce.Surface.prototype.paintAuthor = function ( authorId ) {
	...
	if ( !selection || selection.isNull() ) {
		this.drawSelections( 'otherUser-' + authorId, [] );
		// overlays.$cursor.detach();
		// overlays.$selection.detach();
		return;
	}
