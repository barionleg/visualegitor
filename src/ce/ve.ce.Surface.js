...
/**
 * Create a paste handler for the surface
 *
 * @return {ve.ce.PasteHandler}
 */
ve.ce.Surface.prototype.createPasteHandler = function () {
	return new ve.ce.PasteHandler( this );
};
 ...
 * @return {ve.ce.PasteHandler}
 */
ve.ce.Surface.prototype.getPasteHandler = function () {
...
ve.ce.Surface.prototype.focus = function () {
	...
	// Focus the contentEditable for text selections, or the paste handler for focusedNode selections
	if ( selection.isFocusedNode() ) {
		this.pasteHandler.$element[ 0 ].focus();
...
ve.ce.Surface.prototype.onFocusChange = function () {
	...
	if ( this.deactivated ) {
		...
		if ( hasFocus && !this.isFocused() ) {
			...
			// Move the cursor back to paste handler as we do when focusableNode initially selected.
			// Without this, arrow key navigation from the focusable node would stop working.
			this.preparePasteHandlerForCopy();
		}
	}
};
...
ve.ce.Surface.prototype.onDocumentMouseDown = function ( e ) {
	if ( e.which !== OO.ui.MouseButtons.LEFT ) {
		if ( e.which === OO.ui.MouseButtons.MIDDLE ) {
			this.pasteHandler.prepareForMiddleClickPaste( e );
		}
		return;
	}
...
ve.ce.Surface.prototype.handleDataTransferItems = function ( items, isPaste, targetFragment ) {
	...
	for ( let i = 0, l = items.length; i < l; i++ ) {
		const item = items[ i ];
		const name = dataTransferHandlerFactory.getHandlerNameForItem( item, isPaste, this.pasteHandler.isPasteSpecial() );
		if ( name ) {
...
ve.ce.Surface.prototype.onModelSelect = function () {
	...
	if ( selection instanceof ve.dm.LinearSelection ) {
		...
		if ( this.isDeactivated() && !this.isShownAsDeactivated() && !blockSlug && !focusedNode ) {
			// If deactivated without showing (e.g. by preparePasteHandlerForCopy),
...
/**
 * Prepare the paste handler for a copy event by selecting some text
 *
 * @param {boolean} force Force a native selection, even on mobile (used for click-to-copy)
 */
ve.ce.Surface.prototype.preparePasteHandlerForCopy = function ( force ) {
	...
	// pasteHandler#onCopy will ignore this native selection and use the DM selection
	if ( force || !OO.ui.isMobile() ) {
		this.pasteHandler.prepareForCopy();
	} else {
		// Selecting the paste handler fails on mobile:
...
ve.ce.Surface.prototype.findAdjacentUneditableBranchNode = function ( direction ) {
	...
	let node = $( this.nativeSelection.focusNode ).closest(
		'.ve-ce-branchNode,.ve-ce-leafNode,.ve-ce-surface-pasteHandler'
	)[ 0 ];
	if ( !node || node.classList.contains( 've-ce-surface-pasteHandler' ) ) {
		return null;
	}
