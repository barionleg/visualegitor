...
ve.ce.Surface.prototype.afterDocumentKeyDown = function ( e ) {
	...
	function getSurroundingFocusableNode( node, offset, direction ) {
		var focusableNode, $ceNode;
		if ( node.nodeType === Node.TEXT_NODE ) {
			focusableNode = node;
		} else if ( direction > 0 && offset < node.childNodes.length ) {
			focusableNode = node.childNodes[ offset ];
		} else if ( direction < 0 && offset > 0 ) {
			focusableNode = node.childNodes[ offset - 1 ];
		} else {
			focusableNode = node;
		}
		$ceNode = $( focusableNode ).closest(
			'[contenteditable], .ve-ce-branchNode'
		);
		if ( $ceNode.prop( 'contenteditable' ) !== 'false' ) {
			return undefined;
		}
		return $ceNode.closest(
	...
	if (
		...
		// Observe the direction of motion
		direction = ve.compareDocumentOrder(
			this.nativeSelection.focusNode,
		...
		if ( !focusableNode && Math.abs( offsetDiff ) === 2 ) {
			// Test whether we crossed a focusable node
			focusableNode = this.model.documentModel.documentNode.getNodeFromOffset(
...
/**
 * Get the directionality at the current focus
