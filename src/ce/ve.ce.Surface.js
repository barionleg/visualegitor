...
ve.ce.Surface = function VeCeSurface( model, surface, options ) {
	...
	this.nativeSelection = this.getElementDocument().getSelection();
...
ve.ce.Surface.prototype.getSelectionBoundingClientRect = function () {
	var boundingRect, surfaceRect, sel;
	...
	try {
		boundingRect = this.nativeSelection.getRangeAt( 0 ).getBoundingClientRect();
		return boundingRect;
	} catch ( e ) {
		return null;
	}
...
ve.ce.Surface.prototype.showSelection = function ( range ) {
	...
	this.nativeSelection.removeAllRanges();
	if ( selection.end ) {
		nativeRange.setStart( selection.start.node, selection.start.offset );
		nativeRange.setEnd( selection.end.node, selection.end.offset );
		if ( selection.isBackwards && this.nativeSelection.extend ) {
