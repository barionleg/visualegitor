...
ve.ce.Surface.prototype.handleObservedChanges = function ( oldState, newState ) {
	...
	setTimeout( function () {
		surface.maybeSetBreakpoint( !!insertedText );
	} );
};
 ...
 *
 * @param {boolean} textChanged Whether the observed change included content changes
 */
ve.ce.Surface.prototype.maybeSetBreakpoint = function ( textChanged ) {
	var offset,
	...
	if ( !textChanged ) {
		// Text didn't change, but we were called. This probably means the
		// selection changed. So, fire off a breakpoint anyway.
