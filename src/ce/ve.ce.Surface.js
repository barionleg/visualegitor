...
ve.ce.Surface.prototype.updateActiveAnnotation = function () {
	var viewNode,
		activeAnnotation = this.annotationAtFocus();
	if ( activeAnnotation === this.activeAnnotation ) {
		return;
	}
	if ( this.activeAnnotation && ( viewNode = $.data( this.activeAnnotation, 'view' ) ) ) {
		viewNode.toggleActive( false );
	}
	this.activeAnnotation = activeAnnotation;
	if ( activeAnnotation && ( viewNode = $.data( activeAnnotation, 'view' ) ) ) {
		viewNode.toggleActive( true );
	}
