...
ve.ce.Surface.prototype.afterMutations = function ( mutationRecords ) {
	var removals, nearby, i, iLen, mutationRecord, j, jLen, view, node, tx;

	function addNearby( nodeOrNull ) {
		var branchNode, nearbyView;
		if ( !nodeOrNull ) {
			return;
		}
		branchNode = $( nodeOrNull ).closest( '.ve-ce-branchNode' )[ 0 ];
		if ( !branchNode ) {
			return;
		}
		nearbyView = $.data( branchNode, 'view' );
		if ( nearbyView ) {
			nearby.add( nearbyView );
		}
	}
	removals = [];
	nearby = new Set();
	for ( i = 0, iLen = mutationRecords.length; i < iLen; i++ ) {
		...
		addNearby( mutationRecord.previousSibling );
		addNearby( mutationRecord.nextSibling );
	...
	for ( i = 0, iLen = removals.length; i < iLen; i++ ) {
