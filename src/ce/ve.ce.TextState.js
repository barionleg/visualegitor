...
ve.ce.TextState.prototype.getChangeTransaction = function ( prev, modelDoc, modelOffset, unicornAnnotations ) {
	...
	if ( change.start < Math.min( oldChunks.length, newChunks.length ) ) {
		if ( oldChunks[ change.start ].isEqualElements( newChunks[ change.start ] ) ) {
		...
		if ( oldChunks[ oldChunks.length - 1 - change.end ].isEqualElements(
	...
	for ( i = change.start, iLen = newChunks.length - change.end; i < iLen; i++ ) {
		...
		for ( j = jStart; j < jEnd; j++ ) {
			...
			if ( !oldChunk.isEqualElements( newChunk ) ) {
		...
		if ( annotations === null ) {
			...
			for ( j = jStart; j < jEnd; j++ ) {
				...
				missing = countMissing(
					newChunk.elements,
					oldChunk.elements,
					ve.ce.TextStateChunk.static.isEqualElements
				);
