...
ve.ce.TextState.prototype.getChangeTransaction = function ( prev, modelDoc, modelOffset, unicornAnnotations ) {
	 ...
	 */
	function countMissing( oldArray, newArray, equals ) {
		var i, iLen, j, jLen,
			count = 0;
		for ( i = 0, iLen = oldArray.length; i < iLen; i++ ) {
			for ( j = 0, jLen = newArray.length; j < jLen; j++ ) {
				if ( equals( oldArray[ i ], newArray[ j ] ) ) {
					break;
				}
			}
			if ( j === jLen ) {
	...
	endOffset = tx.pushRemoval(
		modelDoc,
		0,
		// If textStart/textEnd matches cause zero removal, this just does pushRetain
		new ve.Range( changeOffset + textStart, changeOffset + removed - textEnd )
	);

	// Add new content, reusing existing ve.dm.Annotation objects where possible
