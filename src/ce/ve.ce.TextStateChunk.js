 ...
 *
 * @static
 * @param {Node} element1 An annotation element
 * @param {Node} element2 Another annotation element
 * @return {boolean} True if the elements are equal as annotations
 */
ve.ce.TextStateChunk.static.isEqualElements = function ( element1, element2 ) {
 ...
 */
ve.ce.TextStateChunk.prototype.isEqualElements = function ( other ) {
	var i, len;
	...
	for ( i = 0, len = this.elements.length; i < len; i++ ) {
		if ( !this.constructor.static.isEqualElements( this.elements[ i ], other.elements[ i ] ) ) {
			return false;
		}
	}
	return true;
...
ve.ce.TextStateChunk.prototype.isEqual = function ( other ) {
	return this.text === other.text && this.type === other.type && this.isEqualElements( other );
};
