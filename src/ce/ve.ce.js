 ...
 */
ve.ce.dmChangeFromContentChange = function ( oldState, newState ) {
	...
	if ( node.unicornAnnotations ) {
		...
		// Note for collapsed cursors, this ignores whether the cursor was after
		// annotation boundaries, because the change was not at the cursor and so
		// the annotations at the cursor may not be relevant.
		// TODO: Explore whether it is safe to assume that the cursor is inside the
		// same DOM text node, and therefore has relevant annotations
		annotations = modelData.getInsertionAnnotationsFromRange( replacementRange );
	}
	if ( annotations.getLength() ) {
		ve.dm.Document.static.addAnnotationsToData( data, annotations );
	}
