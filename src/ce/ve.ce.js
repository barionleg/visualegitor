 ...
 * @returns {ve.FrozenSelection} Adjust selection; may be the same extent as the argument
 */
ve.ce.adjustLinkSelection = function ( selection, direction ) {
	...
	if ( selection.focusNode === null || selection.isCollapsed ) {
		if ( selection instanceof ve.FrozenSelection ) {
			return selection;
		}
		return new ve.FrozenSelection( selection );
	}
