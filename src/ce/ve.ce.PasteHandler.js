/*!
 * VisualEditor ContentEditable PasteHandler class.
 *
 ...
 */
ve.ce.PasteHandler = function VeCePasteHandler( surface ) {
	// Parent constructor
	ve.ce.PasteHandler.super.call( this );
	...
	this.$element
		.addClass( 've-ce-surface-pasteHandler' )
		// T283853
...
OO.inheritClass( ve.ce.PasteHandler, OO.ui.Element );
 ...
 * @type {string[]}
 */
ve.ce.PasteHandler.static.unsafeAttributes = [
	// Support: Firefox
 ...
 * @type {string}
 */
ve.ce.PasteHandler.static.clipboardKeyMimeType = 'application/x-vnd.wikimedia.visualeditor.clipboardkey';
 ...
 * @return {string} Hash
 */
ve.ce.PasteHandler.static.getClipboardHash = function ( $elements, beforePasteData ) {
 ...
 */
ve.ce.PasteHandler.prototype.getSurface = function () {
	return this.surface;
};
 ...
 */
ve.ce.PasteHandler.prototype.onCut = function ( e ) {
 ...
 */
ve.ce.PasteHandler.prototype.onCopy = function ( e, selection ) {
	...
	// paste handler element (e.g. plain spans) so we must protect against this
 ...
 */
ve.ce.PasteHandler.prototype.getBeforePasteAnnotationSet = function () {
 ...
 */
ve.ce.PasteHandler.prototype.onPaste = function ( e ) {
 ...
 * @param {jQuery.Event} e Paste event
 */
ve.ce.PasteHandler.prototype.beforePaste = function ( e ) {
	...
	// Save scroll position before changing focus to "offscreen" paste handler element
	...
	if ( startNode.canContainContent() ) {
		...
		// to the paste handler element to give CE some context.
		...
		// Giving the paste handler element focus too late can cause problems in FF (!?)
		// so do it up here.
	...
	// Restore scroll position after focusing the paste handler element
 ...
 */
ve.ce.PasteHandler.prototype.afterPaste = function () {
	const surface = this.getSurface(),
		surfaceModel = surface.getModel(),
		documentModel = surfaceModel.getDocument(),
		fragment = surfaceModel.getFragment(),
	...
	// Immedately remove any <style> tags from the paste handler element that might
...
/**
 * Extract the clipboard key and other relevant data from beforePasteData / the paste handler element
 *
 * @return {ve.ce.ClipboardData} Data
 */
ve.ce.PasteHandler.prototype.afterPasteExtractClipboardData = function () {
	...
	if ( beforePasteData.clipboardKey ) {
		...
		if ( beforePasteData.html ) {
			...
			// Fall back on checking the paste handler element

			// HTML in paste handler element may get wrapped, so use the recursive $.find to look for the clipboard key
 ...
 */
ve.ce.PasteHandler.prototype.afterPasteSanitize = function ( linearData, isMultiline, isExternal ) {
 ...
 */
ve.ce.PasteHandler.prototype.afterPasteImportRules = function ( isMultiline ) {
 ...
 */
ve.ce.PasteHandler.prototype.afterPasteAddToFragmentFromInternal = function ( slice, fragment, targetFragment, isMultiline ) {
 ...
 * @return {jQuery.Promise} Promise which resolves when the content has been inserted
 */
ve.ce.PasteHandler.prototype.afterPasteInsertInternalData = function ( targetFragment, data ) {
 ...
 * @param {ve.dm.SurfaceFragment} targetFragment Fragment to insert into
 * @param {boolean} [isMultiline] Pasting to a multiline context
 * @param {boolean} [forceClipboardData] Ignore the paste handler element, and use only clipboard html
 * @return {jQuery.Promise} Promise which resolves when the content has been inserted
 */
ve.ce.PasteHandler.prototype.afterPasteAddToFragmentFromExternal = function ( clipboardKey, $clipboardHtml, fragment, targetFragment, isMultiline, forceClipboardData ) {
	...
	// would produce much more consistent results, as the paste handler element approach can also re-order
	// and destroy nodes.
	if (
		$clipboardHtml && (
			forceClipboardData ||
 ...
 * @return {jQuery.Promise} Promise which resolves when the content has been inserted
 */
ve.ce.PasteHandler.prototype.afterPasteInsertExternalData = function ( targetFragment, pastedDocumentModel, contextRange ) {
	// Temporary tracking for T362358
 ...
 */
ve.ce.PasteHandler.prototype.afterPasteFromExternalContextRange = function ( pastedDocumentModel, isMultiline, forceClipboardData ) {
	const data = pastedDocumentModel.data,
	...
	if ( ( leftText || rightText ) && !forceClipboardData ) {
		// If any text context is left over, assume the paste handler element got corrupted
...
/**
 * Helper to clean up externally pasted HTML (via paste handler element).
 *
 * @param {jQuery} $element Root element containing pasted stuff to sanitize
 */
ve.ce.PasteHandler.prototype.afterPasteSanitizeExternal = function ( $element ) {
...
/**
 * Place the native selection in the paste handler, ready for a copy
 */
ve.ce.PasteHandler.prototype.prepareForCopy = function () {
...
/**
 * Prepare the paste handler to treat the next paste as "speical" (i.e. plain text)
 */
ve.ce.PasteHandler.prototype.prepareForPasteSpecial = function () {
 ...
 * @return {boolean}
 */
ve.ce.PasteHandler.prototype.isPasteSpecial = function () {
	return this.pasteSpecial;
};

ve.ce.PasteHandler.prototype.prepareForMiddleClickPaste = function ( e ) {
