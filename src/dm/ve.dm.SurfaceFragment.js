		...
		case 'word':
			if ( !oldRange.isCollapsed() ) {
				newRange = ve.Range.static.newCoveringRange( [
					this.document.data.getAdjacentWordRange( oldRange.start ),
					this.document.data.getAdjacentWordRange( oldRange.end )
				], oldRange.isBackwards() );
			} else {
				// optimisation for zero-length ranges
				newRange = this.document.data.getAdjacentWordRange( oldRange.start );
			}
			break;
		case 'annotation':
