...
/**
 * Translate the current selection for one or more transactions, using this.excludeInsertions.
 *
 * @param {ve.dm.Transaction|ve.dm.Transaction[]} txs Transaction(s) to translate for
 * @param {boolean} [noUpdate] Use this.selection directly rather than trying to update it first
 *  Only use this if calling this.update() will lead to problems (e.g. recursion)
 * @returns {ve.dm.Selection} Translated selection
 */
ve.dm.SurfaceFragment.prototype.getTranslatedSelection = function ( txs, noUpdate ) {
	// Handle empty selection
	if ( this.isNull() ) {
		return null;
	}

	// Only translate text selections
	...
	return new ve.dm.LinearSelection( this.getDocument(), range );
};
...
ve.dm.SurfaceFragment.prototype.update = function ( selection ) {
	...
	if ( selection ) {
		...
		this.selection = this.getTranslatedSelection( txs, true );
...
ve.dm.SurfaceFragment.prototype.change = function ( txs, selection ) {
	...
	this.surface.change( txs, !this.noAutoSelect && ( selection || this.getTranslatedSelection( txs ) ) );
	if ( selection ) {
...
ve.dm.SurfaceFragment.prototype.expandLinearSelection = function ( scope, type ) {
	...
	return this.clone(
		newRange ?
			...
			new ve.dm.EmptySelection( this.getDocument() )
	);
};
