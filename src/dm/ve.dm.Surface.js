...
ve.dm.Surface = function VeDmSurface( doc ) {
	...
	this.selection = new ve.dm.EmptySelection( this.getDocument() );
	this.selectionBefore = new ve.dm.EmptySelection( this.getDocument() );
	this.branchNodes = {};
...
ve.dm.Surface.prototype.pushStaging = function ( allowUndo ) {
	...
	this.stagingStack.push( {
		transactions: [],
		selectionBefore: new ve.dm.EmptySelection( this.documentModel ),
		allowUndo: !!allowUndo
	} );
};
...
ve.dm.Surface.prototype.applyStaging = function () {
	...
	if ( this.isStaging() ) {
		...
		// If the current level has an empty selectionBefore, copy that over too
		if ( this.getStaging().selectionBefore.isEmpty() ) {
...
ve.dm.Surface.prototype.setEmptySelection = function () {
	this.setSelection( new ve.dm.EmptySelection( this.getDocument() ) );
};
...
ve.dm.Surface.prototype.setSelection = function ( selection ) {
	...
	if ( selection instanceof ve.dm.LinearSelection ) {
		...
		// Update branch nodes
		branchNodes.start = this.documentModel.getBranchNodeFromOffset( range.start );
		if ( !range.isCollapsed() ) {
			branchNodes.end = this.documentModel.getBranchNodeFromOffset( range.end );
		}
		// Update selected node
		if ( !range.isCollapsed() ) {
			startNode = this.documentModel.documentNode.getNodeFromOffset( range.start + 1 );
...
ve.dm.Surface.prototype.breakpoint = function () {
	...
	if ( this.newTransactions.length > 0 ) {
		...
		this.emit( 'history' );
		return true;
	} else if ( this.selectionBefore.isEmpty() && !this.selection.isEmpty() ) {
...
ve.dm.Surface.prototype.onDocumentTransact = function ( tx ) {
	if ( this.selection instanceof ve.dm.LinearSelection ) {
		this.setLinearSelection( tx.translateRange( this.getSelection().getRange() ) );
	}
