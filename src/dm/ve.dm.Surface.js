...
ve.dm.Surface.prototype.breakpoint = function () {
	...
	if ( this.newTransactions.length > 0 ) {
		this.undoStack.push( {
			start: this.getDocument().getCompleteHistoryLength() -
...
ve.dm.Surface.prototype.undo = function () {
	var item, history, done, result, selection;
	if ( !this.canUndo() ) {
		return;
	}
	...
	if ( item ) {
		...
		if ( result.rejected ) {
			// Rebasing conflict: truncate undo stack
			this.undoStack.splice( 0, this.undoStack.length - this.undoIndex + 1 );
			this.undoIndex = 0;
		} else {
