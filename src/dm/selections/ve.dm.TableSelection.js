 ...
 */
ve.dm.TableSelection.static.getMatrixCells = function ( matrix, selectionOffsets, includePlaceholders ) {
	var row, col, cell,
		cells = [],
		visited = {};
...
ve.dm.TableSelection.prototype.expand = function ( doc ) {
	...
	while ( cells.length > lastCellCount ) {
		...
		cells = this.constructor.static.getMatrixCells( matrix, {
			startCol: startCol,
			startRow: startRow,
			endCol: endCol,
			endRow: endRow
...
ve.dm.TableSelection.prototype.getMatrixCells = function ( doc, includePlaceholders ) {
	return this.constructor.static.getMatrixCells(
