	...
	for ( i = 0; i < ilen; i++ ) {
		for ( j = 0; j < jlen; j++ ) {
			if ( oldIndices[ i ] !== null && newIndices[ j ] !== null ) {
				...
				if ( diffResults && (
					'linearDiff' in diffResults || diffResults.attributeChange ||
					diffResults.treeDiff || diffResults.tableDiff ||
					// List diff
...
ve.dm.VisualDiff.prototype.diffTableNodes = function ( oldNode, newNode ) {
	...
	CellView.prototype.equals = function ( d1, d2 ) {
		var node1 = ( d1 && d1.node ) || null;
		var node2 = ( d2 && d2.node ) || null;
		if ( !node1 || !node2 ) {
			return node1 === node2;
		}
		return visualDiff.compareNodes( node1, node2 );
	};
	CellView.prototype.toString = function ( d ) {
		var node = d && d.node;
		if ( node ) {
			return JSON.stringify( node.getDocument().getData( node.getOuterRange() ), function ( key, value ) {
				if ( value && value.originalDomElementsHash ) {
					var clone = ve.copy( value );
					delete clone.originalDomElementsHash;
					return clone;
				}
				return value;
			} );
	...
	data.forEach( function ( row, rowIndex ) {
		...
		for ( var i = 0; i < n; i++ ) {
			...
			insertRow.forEach( function ( cell, colIndex ) {
				...
				// Cells may have be converted to a diff by the column pass
				var before = removeRow[ colIndex ] && removeRow[ colIndex ].before ? removeRow[ colIndex ].before : removeRow[ colIndex ];
				var after = cell && cell.after ? cell.after : cell;
				insertRow[ colIndex ] = {
					before: before,
					after: after
				};
