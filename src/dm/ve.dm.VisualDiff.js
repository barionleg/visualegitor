 ...
 *
 * @param {ve.dm.Node} oldDocChild Child of the old document node
 * @param {[type]} newDocChild Child of the new document node
 * @return {Array|false} The diff, or false if the children are too different
 */
ve.dm.VisualDiff.prototype.getDocChildDiff = function ( oldDocChild, newDocChild ) {
	...
	function getCleanDiff( diff ) {
		...
		for ( i = 0, ilen = diff.length; i < ilen; i++ ) {
			action = diff[ i ][ 0 ];
			if ( action === -1 ) {
				remove = remove.concat( diff[ i ][ 1 ] );
			} else if ( action === 1 ) {
				insert = insert.concat( diff[ i ][ 1 ] );
			} else if ( action === 0 ) {
				if ( remove.length > 0 ) {
