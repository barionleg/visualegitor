...
/* global diff_match_patch, treeDiff */
...
ve.dm.VisualDiff.prototype.getDocChildDiff = function ( oldDocChild, newDocChild ) {
	...
	for ( i = 0, ilen = treeDiff.length; i < ilen; i++ ) {
		...
		if ( treeDiff[ i ][ 0 ] !== null && treeDiff[ i ][ 1 ] !== null ) {
			...
			// If we got this far, they are both CBNs
				...
				// Do a linear diff to find out whether content has changed
	...
	diffLength = diffLength === 0 ? diffLength + 0.0001 : diffLength;
	if ( keepLength / diffLength < 0.5 ) {
		return false;
	}
	return {
		treeDiff: treeDiff,
