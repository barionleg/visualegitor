...
ve.dm.VisualDiff = function VeDmVisualDiff( oldDocOrNode, newDocOrNode, timeout ) {
	...
	this.history = newDoc.completeHistory;
...
ve.dm.VisualDiff.prototype.diffTreeNodes = function ( oldTreeNode, newTreeNode ) {
	...
	function getHeuristicTransaction( oldNode, newNode ) {
		var tx, oldRange, newRange, oldData, newData, differ, diff;
		function repeat( s, n ) {
	...
	function getTransactionFromHistory( oldNode, newNode, history ) {
		// TODO: relativise the history to this node
		var tx, activeRange, data;
		tx = history.squash().transactions[ 0 ];
		oldRange = oldNode.getRange();
		activeRange = tx.getActiveRangeAndLengthDiff();
		if (
			oldRange.start >= activeRange.start ||
			oldRange.end <= activeRange.end
		) {
			throw new Error( 'TODO: relativise the history to this node' );
		}
		return tx;
	}
	...
	// HACK HACK OUTRAGEOUS HACK: assume only this tree node got changed
	tx = getTransactionFromHistory( oldTree.root.node, newTree.root.node, this.history );
