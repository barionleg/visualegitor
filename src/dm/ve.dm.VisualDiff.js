 ...
 *
 * @param {Array} oldNodes Nodes from the old document
 * @param {Array} newNodes Nodes from the new document
 ...
 *
 * @param {Array} oldIndices Indices of the old nodes
 * @param {Array} newIndices Indices of the new nodes
 * @param {Array} oldNodes Nodes from the old document
 * @param {Array} newNodes Nodes from the new document
...
ve.dm.VisualDiff.prototype.diffNodes = function ( oldNode, newNode ) {
	...
	if ( oldNode.isLeafLike() ) {
		diff = this.diffLeafNodes( oldNode, newNode );
	} else if ( oldNode.isListLike() ) {
...
ve.dm.VisualDiff.prototype.diffListNodes = function ( oldNode, newNode ) {
	...
	// TODO: Check list type as well as depth
	// TODO: neaten this
	for ( i in listDiff.oldToNew ) {
		j = listDiff.oldToNew[ i ];
		j = typeof j === 'number' ? j : j.node;
		if ( j && !( ve.compare( oldFlatList.metadata[ i ].depth, newFlatList.metadata[ j ].depth ) ) ) {
			// TODO: Neaten this
			if ( typeof listDiff.oldToNew[ i ] === 'number' ) {
				...
				listDiff.oldToNew[ i ].diff.metadata = {
					oldMetadata: oldFlatList.metadata[ i ],
					newMetadata: newFlatList.metadata[ j ]
				};
			}
