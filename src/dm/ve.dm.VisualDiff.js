...
ve.dm.VisualDiff = function VeDmVisualDiff( oldDocOrNode, newDocOrNode, timeout ) {
	...
	this.diff = {
		...
		internalListDiff: {
			groups: this.getInternalListDiffGroups( oldInternalList, newInternalList ),
			oldNode: oldInternalList.getListNode(),
			newNode: newInternalList.getListNode()
		}
	};
 ...
 */
ve.dm.VisualDiff.prototype.getInternalListDiffGroups = function ( oldInternalList, newInternalList ) {
	...
	for ( var i = 0, ilen = groups.length; i < ilen; i++ ) {
		...
		switch ( group.action ) {
			case 'diff':
				...
				if ( diffInfo ) {
					internalListDiffGroups[ group.group ] = diffInfo;
				}
			...
			case 'insert':
				...
				internalListDiffGroups[ group.group ] = getInternalListItems(
					newDocNodeGroups[ group.group ].indexOrder,
					newDocInternalListNode.children,
					1
				);
				internalListDiffGroups[ group.group ].changes = true;
				break;
