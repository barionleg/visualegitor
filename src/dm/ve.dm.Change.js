 ...
 * @param {ve.dm.IndexValueStore} store New annotations appearing in this change
 */
ve.dm.Change = function VeDmChange( start, transactions, store ) {
	this.start = start;
...
ve.dm.Change.static.deserialize = function ( doc, data ) {
	...
	return new ve.dm.Change( data.start, data.transactions.map( function ( operations ) {
		return new ve.dm.Transaction( doc, operations );
	} ), store );
};
 ...
 * @return {ve.dm.Change} Sliced change, with adjusted start offset
 */
ve.dm.Change.prototype.slice = function ( start, end ) {
	var omit = this.constructor.static.getHashes( this.transactions.slice( 0, start ) );
	return new ve.dm.Change(
		this.start + start,
		this.transactions.slice( start, end ),
		this.store.difference( omit )
	);
};
