 ...
 */
ve.dm.TransactionSquasher.static.annotateData = function ( array, offset, len ) {
	...
	for ( i = offset; i < len; i++ ) {
		...
		for ( j = 0, jLen = this.changes.length; j < jLen; j++ ) {
			...
			if ( change.method === 'set' ) {
				if ( annotations.indexOf( change.hash ) !== undefined ) {
...
/**
 * Add annotation start/stop operations as required, so that annotation changes are incorporated
 * to cover the transaction's retain ranges, but not its replace ranges.
 *
 * @param {Object} change The annotation change to be switched
 * @param {boolean} change.active Whether the annotation is active; will be modified in-place
 * @param {boolean} wantActive True to activate the annotation changes; false to deactivate them
 */
ve.dm.TransactionSquasher.prototype.switchAnnotations = function ( change, wantActive ) {
...
ve.dm.TransactionSquasher.prototype.activateAnnotations = function () {
	...
	for ( i = 0, iLen = this.changes.length; i < iLen; i++ ) {
		this.switchAnnotations( this.changes[ i ], true );
	}
};
...
ve.dm.TransactionSquasher.prototype.deactivateAnnotations = function () {
	var i, iLen;
	for ( i = 0, iLen = this.changes.length; i < iLen; i++ ) {
		this.switchAnnotations( this.changes[ i ], false );
	}
};
...
ve.dm.TransactionSquasher.prototype.processAnnotation = function ( method, bias, hash, spliceAt ) {
	...
	if ( bias === 'start' ) {
		...
		this.switchAnnotations( change, true );
	} else {
		...
		this.switchAnnotations( change, false );
