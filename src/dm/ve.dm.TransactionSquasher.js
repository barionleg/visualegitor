...
ve.dm.TransactionSquasher = function VeDmTransactionSquasher( transaction ) {
	...
	/**
	 * During squashIn, annotation changes from the *existing* transaction that should
	 * be applied in reverse to content being removed
	 *
	 * @property {Object[]} oldChanges Annotations to set/clear on removed content
	...
	/**
	 * @property {Object|null} During squashIn, the open element at current offset, if any
	 */
	this.openElement = null;

	/**
	 * @property {Object} During squashIn, live references to attribute operations at
	 * current offset, keyed by attribute name, or null if at an open element
	 */
...
ve.dm.TransactionSquasher.prototype.processAttribute = function ( key, from, to ) {
	...
	if ( this.openElement ) {
		...
		ob = this.attributeOperations[ key ];
		if ( ob && ob.to !== from ) {
			throw new Error( 'Unexpected prior attribute value' );
		}
		if ( ob ) {
			...
			this.operations.splice( this.index, 0, ob[ key ] );
			this.index++;
		}
	}
};
