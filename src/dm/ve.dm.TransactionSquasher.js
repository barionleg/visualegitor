...
ve.dm.TransactionSquasher = function VeDmTransactionSquasher( transaction ) {
	...
	/**
	 * @property {number[]} setSpliceAt Each annotation's spliceAt value
	 */
	this.setSpliceAt = [];

	/**
	 * @property {boolean[]} setIsActive Whether each annotation is open in this transaction
	 */
	this.setIsActive = [];
	...
	/**
	 * @property {number[]} clearSpliceAt Each annotation's spliceAt value
	 */
	this.clearSpliceAt = [];

	/**
	 * @property {boolean} clearIsActive Whether each annotation is open in this transaction
	 */
	this.clearIsActive = [];
};
...
/**
 * Start/stop annotations as required, by adding annotation start/stop operations
 *
 * @param {string} method The method, set|clear
 * @param {string} bias The bias, start|stop
 * @param {string[]} annotations Hash for each annotation
 * @param {number[]} spliceAt Annotation array index at which to splice each annotation
 * @param {boolean[]} isActive Array containing each annotations status (modified in-place)
 */
ve.dm.TransactionSquasher.prototype.switchAnnotations = function ( method, bias, annotations, spliceAt, isActive ) {
	var i, iLen,
		wantActive = bias === 'start';
	for ( i = 0, iLen = annotations.length; i < iLen; i++ ) {
		if ( isActive[ i ] !== wantActive ) {
			if ( this.offset > 0 ) {
				this.splitRetain();
			}
			...
			this.index++;
			isActive[ i ] = wantActive;
		}
	}
};

ve.dm.TransactionSquasher.prototype.activateAnnotations = function () {
	this.switchAnnotations( 'set', 'start', this.setAnnotations, this.setSpliceAt, this.setIsActive );
	this.switchAnnotations( 'clear', 'start', this.clearAnnotations, this.clearSpliceAt, this.clearIsActive );
};

ve.dm.TransactionSquasher.prototype.deactivateAnnotations = function () {
	this.switchAnnotations( 'set', 'stop', this.setAnnotations, this.setSpliceAt, this.setIsActive );
	this.switchAnnotations( 'clear', 'stop', this.clearAnnotations, this.clearSpliceAt, this.clearIsActive );
};
...
ve.dm.TransactionSquasher.prototype.processAnnotation = function ( method, bias, hash, spliceAt ) {
	var annotations, annSpliceAt, annIsActive, i;
	if ( method === 'set' ) {
		annotations = this.setAnnotations;
		annSpliceAt = this.setSpliceAt;
		annIsActive = this.setIsActive;
	} else {
		annotations = this.clearAnnotations;
		annSpliceAt = this.clearSpliceAt;
		annIsActive = this.clearIsActive;
	}
	// TODO: Need to move spliceAt to adjust for other annotation changes going on

	if ( bias === 'start' ) {
		annotations.push( hash );
		annSpliceAt.push( spliceAt );
		annIsActive.push( false );
	} else {
		i = annotations.lastIndexOf( hash );
		if ( i === -1 ) {
			throw new Error( 'Bad annotation order' );
		}
		this.switchAnnotations(
			method,
			bias,
			[ annotations[ i ] ],
			[ annSpliceAt[ i ] ],
		...
		annSpliceAt.splice( i, 1 );
		annIsActive.splice( i, 1 );
	}
...
/**
 * If in the interior of a retain operation, to split it in two here without moving
 *
