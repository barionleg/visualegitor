...
ve.dm.TransactionSquasher.prototype.processRemove = function ( items ) {
	...
	if ( this.op.type === 'attribute' ) {
		...
		items[ 0 ] = this.changedElement( items[ 0 ], this.op.key, this.op.to, this.op.from );
		this.operations.splice( this.index, 1 );
		this.op = this.operations[ this.index ];
		// By assumption, this.offset is already 0
		this.tryUnsplit();
...
ve.dm.TransactionSquasher.prototype.processAttribute = function ( key, from, to ) {
	...
	if ( this.op && this.op.type === 'replace' && this.offset < this.op.insert.length ) {
		...
		this.op.insert[ this.offset ] = this.changedElement( item, key, from, to );
	} else {
...
/**
 * Clone an open element, with an attribute change applied
 *
 ...
 * @return {Object} Clone of the open element, with the attribute change applied
 */
ve.dm.TransactionSquasher.prototype.changedElement = function ( openElement, key, from, to ) {
	openElement = JSON.parse( JSON.stringify( openElement ) );
