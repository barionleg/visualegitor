 ...
 * @param {number[]} setAnnotations Annotation indexes to set
 * @param {number[]} clearAnnotations Annotation indexes to clear
 */
ve.dm.TransactionSquasher.static.annotateData = function ( array, offset, len, setAnnotations,
	clearAnnotations ) {
	var i, item,
		annotations;
	...
	for ( i = offset; i < len; i++ ) {
		...
		// Remove clearAnnotations
		debugger;
		// Append setAnnotations
	}
};
...
ve.dm.TransactionSquasher.prototype.squashIn = function ( tx ) {
	...
	for ( i = 0, iLen = tx.operations.length; i < iLen; i++ ) {
		...
		if ( op.type === 'retain' ) {
			...
			this.processAnnotation( op.method, op.bias, op.index );
		} else {
...
ve.dm.TransactionSquasher.prototype.processRetain = function ( maxLength ) {
	...
	if ( this.op.type === 'replace' ) {
		...
		this.constructor.static.annotateData( this.op.insert, this.offset, len,
			this.setAnnotations, this.clearAnnotations );
		this.offset += len;
		return len;
	}
...
ve.dm.TransactionSquasher.prototype.processRemove = function ( items ) {
	...
	if ( this.op.type === 'replace' ) {
		...
		for ( i = 0; i < len; i++ ) {
			if ( items[ i ] !== this.op.insert[ this.offset + i ] ) {
 ...
 * @param {string} index Hash value of the annotation
 */
ve.dm.TransactionSquasher.prototype.processAnnotation = function ( method, bias, index ) {
	var annotations = method === 'set' ?
		this.setAnnotations :
		this.clearAnnotations;
	...
	if ( bias === 'start' ) {
		...
		if ( annotations[ annotations.length - 1 ] !== index ) {
			throw new Error( 'Bad annotation order' );
		}
		// TODO: we can't safely pop because we don't know whether this is the top one
		annotations.pop();
	}
