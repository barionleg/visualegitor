...
ve.dm.ElementLinearData.static.replaceAnnotationHashesForItem = function ( item, hashes ) {
	...
	if ( hashes.length > 0 ) {
		if ( isElement ) {
			...
			character = ve.dm.ElementLinearData.static.getCharacterData( item );
		...
		if ( isElement ) {
			...
			item = ve.dm.ElementLinearData.static.getCharacterData( item );
		}
	}
	return item;
};
 ...
 */
ve.dm.ElementLinearData.static.getCharacterData = function ( item ) {
...
ve.dm.ElementLinearData.prototype.getCharacterData = function ( offset ) {
	var item = this.getData( offset );
	return this.constructor.static.getCharacterData( item );
};
