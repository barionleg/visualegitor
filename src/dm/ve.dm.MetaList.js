...
ve.dm.MetaList.prototype.insertMeta = function ( meta, offset ) {
	...
	tx = ve.dm.Transaction.newFromInsertion( this.document, offset, [ meta, closeMeta ] );
...
ve.dm.MetaList.prototype.removeMeta = function ( item ) {
	...
	tx = ve.dm.Transaction.newFromRemoval(
		this.document,
		// Range 2 to remove the meta item and its close tag
		new ve.Range( offset, offset + 2 )
	);
	this.surface.change( tx );
};
