 ...
 */
ve.dm.Transaction.prototype.rebasedOnto = function ( other ) {
	var i, len, op, range, bothCollapsed,
		changedRanges = ve.dm.Transaction.prototype.getChangedRanges.call( other ),
		...
		r = 0,
		tx = ve.dm.Transaction.prototype.clone.call( this );
	...
	for ( i = 0, len = tx.operations.length; i < len; i++ ) {
		...
		while ( true ) {
			...
			bothCollapsed = opEnd === opStart && range.end === range.start;
			if ( op.type !== 'retain' && opEnd === range.start && !bothCollapsed ) {
				// Non-retain op touches the edge of range; move to the next operation
				continue opLoop;
			}

			// Else this range overlaps with op. This includes the case where range
			// is zero length, so an op that ends at an insertion point will get
			// resized. TODO check all cases
