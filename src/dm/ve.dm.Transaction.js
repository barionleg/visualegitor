 ...
 * this implementation, transactions are considered to conflict if they modify ranges that
 * overlap, where a transaction's "modified range" means the smallest range outside which the
 * start document and the end document are identical. (The identity transaction on a document
 * state has no modified range and is trivially rebasable with any parallel transaction).
 *
 * For non-conflicting transactions, rebasing of each transaction is performed by resizing the
 * unmodified range either before or after the transaction to accommodate the length difference
 * caused by the other transaction. There is ambiguity in the case where both transactions have
 * a zero-length modified range at the same position (i.e. two inserts in the same place); in
 * this case transactionA's insertion is put before transactionB's.
 *
 * It is impossible for rebasing defined this way to create an invalid transaction that breaks
 * tree validity. This is clear because every position in the rebased transaction's modified
 ...
 * transactions' modified ranges do not overlap).
 *
