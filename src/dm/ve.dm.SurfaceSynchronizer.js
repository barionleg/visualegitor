...
ve.dm.SurfaceSynchronizer = function VeDmSurfaceSynchronizer( surface ) {
	...
	this.author = Math.random(); // TODO have the server assign this
...
ve.dm.SurfaceSynchronizer.prototype.onModelSelect = function () {
	this.submitChangeDebounced();
};
...
ve.dm.SurfaceSynchronizer.prototype.submitChange = function () {
	...
	console.log( this.author, 'submitChange', change.serialize(), selection.getDescription() );
};
...
ve.dm.SurfaceSynchronizer.prototype.onNewChange = function ( data ) {
	...
	console.log( this.author, 'newChange', data );
	...
	if ( this.submittedChange ) {
		if ( data.author === this.author ) {
			...
			if ( unsentRebased ) {
				...
				console.log( this.author, 'rebase of unsent changes succeeded' );
				// Apply incoming
				...
				console.log( this.author, 'rebase of unsent changes failed' );
