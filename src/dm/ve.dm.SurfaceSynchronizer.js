...
ve.dm.SurfaceSynchronizer = function VeDmSurfaceSynchronizer( surface ) {
	...
	// Debouce this with a 0ms timeout so we always wait until the end of the event loop before
	// submitting an update. This prevents us from doing things like transmitting a transaction
	// before the selection is updated, or transmitting only part of a synchronously committed
	// sequence of transactions. Also throttle at 250ms so we don't transmit too often.
...
ve.dm.SurfaceSynchronizer.prototype.applyNewSelections = function ( newSelections, changeOrTx ) {
	var author, translatedSelection,
		...
		changed;
	console.log( 'applying selections', changeOrTx );
	for ( author in newSelections ) {
		...
		// FIXME: ??? Why is the result of this comparison never used?
