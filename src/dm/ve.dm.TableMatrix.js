...
ve.dm.TableMatrix.prototype.getCellsForRectangle = function ( rect ) {
	...
	for ( row = rect.start.row; row <= rect.end.row; row++ ) {
		for ( col = rect.start.col; col <= rect.end.col; col++ ) {
			...
			if ( cell.constructor.static.type === 'placeholder' ) {
				cell = cell.owner;
			}
			if ( !visited[cell.key] ) {
				cells.push( cell );
...
ve.dm.TableMatrix.prototype.findClosestCell = function ( cell ) {
	...
	for (col = cell.col; col >= 0; col--) {
		if ( rowCells[col].constructor.static.type === 'cell' ) {
			return rowCells[col];
		}
	}
 ...
 */
ve.dm.TableMatrixCell = function Cell( node, row, col ) {
	this.node = node;
	this.row = row;
	this.col = col;
	this.key = row + '_' + col;
...
ve.dm.TableMatrixCell.static.type = 'cell';
 ...
 * @param {Number} col Column index
 */
ve.dm.TableMatrixPlaceholder = function PlaceHolder( owner, row, col ) {
...
ve.dm.TableMatrixPlaceholder.static.type = 'placeholder';
