...
ve.dm.TableCellableNode.static.areNodesCellable = function ( domNodes ) {
	...
	return domNodes.every( function ( node ) {
		return cellElems.has( node.nodeName.toLowerCase() ) &&
...
ve.dm.TableCellableNode.static.setAttributes = function ( attributes, domElements ) {
	...
	for ( let i = 1; i < domElements.length; i++ ) {
		...
		const colspan = domElem.getAttribute( 'colspan' );
		if ( colspan !== null && colspan !== '' && !isNaN( Number( colspan ) ) ) {
			attributes.computedColspan += Number( colspan );
		} else {
