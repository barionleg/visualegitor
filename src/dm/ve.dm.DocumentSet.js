...
ve.dm.DocumentSet.prototype.getUniqueName = function ( group, prefix, oldName ) {
	var uniqueName, groupObj = this.getGroupObj( group ), num = 0;
	if ( prefix === undefined ) {
		prefix = '';
	}
	...
	// an existing name
	while ( groupObj.indexes[prefix + num] !== undefined || groupObj.usedUniqueNames[prefix + num] ) {
		num++;
	}
...
ve.dm.DocumentSet.prototype.getNamePosition = function ( group, name ) {
	...
	var index = this.documentGroups[group].order.indexOf( name );
