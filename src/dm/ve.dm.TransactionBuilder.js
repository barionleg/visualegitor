...
/**
 * Find the maximum tree height reached in the data, relative to the start height
 *
 * For example, a height of 2 means the highest point in the data is at the same level as
 * the grandparent of the start node
 *
 * @param {Array} data The linear data to scan
 * @return {number} The maximum height reached, relative to the start height
 */
ve.dm.TransactionBuilder.static.getMaxTreeHeight = function ( data ) {
	var i, len,
		treeHeight = 0,
		maxTreeHeight = 0;
	for ( i = 0, len = data.length; i < len; i++ ) {
		if ( !ve.dm.LinearData.static.isElementData( data[ i ] ) ) {
			continue;
		}
		treeHeight += ve.dm.LinearData.static.isOpenElementData( data[ i ] ) ? -1 : 1;
...
ve.dm.TransactionBuilder.prototype.pushReplacement = function ( doc, offset, removeLength, insert, removeMetadata, insertedDataOffset, insertedDataLength ) {
	...
	if (
		collapse.length > 0 &&
		( rise = this.constructor.static.getMaxTreeHeight( remove ) ) > 0
	) {
		// Metadata at this position might be invalid; fixup
		this.pushMeta( doc, offset, rise, collapse );
		collapse = [];
	} else {
		insert = collapse.concat( insert );
 ...
 * @param {number} rise Unused TODO: should we use it?
 * @param {Array} metaItems linear data containing just meta items
 */
ve.dm.TransactionBuilder.prototype.pushMeta = function ( doc, offset, rise, metaItems ) {
	var i, op, items, j, type, position,
