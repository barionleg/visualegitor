...
ve.dm.TransactionBuilder.static.newFromWrap = function ( doc, range, unwrapOuter, wrapOuter, unwrapEach, wrapEach ) {
	...
	function match( direction, offset, matchList, matchName ) {
		var start, stop, step, stopOffset, i, item;
		if ( direction === 'forwards' ) {
			start = 0;
			stop = matchList.length;
			step = 1;
			stopOffset = doc.data.data.length;
	...
	if ( wrapEach.length === 0 && unwrapEach.length === 0 ) {
		...
		for ( i = range.start; i < range.end; i++ ) {
			...
			txBuilder.pushReplacement( doc, endOffset, ptr - i, ve.copy( closingWrapEach ) );
			// Goto end of wrapper
			if ( ptr > i ) {
				i = ptr - 1;
			}
	...
	txBuilder.pushReplacement( doc, range.end, unwrapOuter.length, closingArray( wrapOuter ) );
