 ...
 */
ve.dm.TreeModifier.static.applyTreeOperations = function ( document, treeOps ) {
	var i, iLen;
	for ( i = 0, iLen = treeOps.length; i < iLen; i++ ) {
		this.applyTreeOperation( document, treeOps[ i ] );
	}
};
 ...
 */
ve.dm.TreeModifier.static.applyTreeOperation = function ( document, treeOp ) {
	var wantText, f, t, a, data, node;
	...
	function spliceLinear( offset, remove, data ) {
		data = data || [];
		return ve.batchSplice( document.data, offset, remove, data );
	}
	...
	if ( treeOp.type === 'removeNode' ) {
		...
		spliceLinear( a.linearOffset, 0, treeOp.balancedData );
		a.node.splice( a.offset, ve.dm.nodeFactory.createFromElement( treeOp.element ) );
	} else if ( treeOp.type === 'moveNode' ) {
		data = spliceLinear( f.linearOffset, f.node.getOuterLength() );
		node = f.node.splice( f.offset, 1 );
		spliceLinear( t.linearOffset, 0, data );
...
ve.dm.TreeModifier.prototype.process = function ( document, transaction ) {
	this.setup( document );
	// XXX need isReversed and markBranchNodeChanged ... and maybe all that undo stuff
	this.calculateTreeOperations( transaction );
	this.constructor.static.applyTreeOperations( document, this.treeOps );
};
...
ve.dm.TreeModifier.prototype.cursorsMatch = function () {
	...
	return this.insertedPositions.length === 0 &&
		JSON.stringify( removerPosition ) === JSON.stringify( inserterPosition );
};
...
ve.dm.TreeModifier.prototype.processRetain = function ( maxLength ) {
	...
	if ( removerStep.type === 'crosstext' ) {
		...
		this.pushTreeOp( {
			type: 'moveNode',
			isContent: this.isContent( removerStep.node ),
			from: this.getRemoverPosition( removerStep ),
			to: this.getInserterPosition()
		} );
		if ( this.insertedPositions.length ) {
		...
		this.pushTreeOp( {
			type: 'insertNode',
			isContent: this.isContent( this.inserter.node ),
...
ve.dm.TreeModifier.prototype.pushRemoveLast = function () {
	...
	this.pushTreeOp( {
		type: 'removeNode',
		isContent: this.isContent( step.node ),
		at: this.getRemoverPosition( step ),
...
ve.dm.TreeModifier.prototype.isContent = function ( node ) {
