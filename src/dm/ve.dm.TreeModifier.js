...
ve.dm.TreeModifier.prototype.ensureTextNode = function () {
	...
	if ( this.inserter.node.children[ this.inserter.offset ] instanceof ve.dm.TextNode ) {
		...
		// The previous node is a text node; move backwards and step in
		if ( this.cursorsMatch() ) {
			this.remover.offset--;
			this.remover.stepIn();
...
ve.dm.TreeModifier.prototype.processInsert = function ( data ) {
	var step,
		inserter = this.inserter,
		type = data.type ? ( data.type[ 0 ] === '/' ? 'close' : 'open' ) : 'crosstext';
	if ( type === 'open' ) {
		if ( inserter.node instanceof ve.dm.TextNode ) {
			// Step out of the text node, even if that skips past some content (in
			// which case the remover logically must later cross that content in
			// either processRemove or processRetain).
			inserter.stepOut();
		}
		this.create( data );
		inserter.stepIn();
