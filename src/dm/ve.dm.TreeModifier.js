...
ve.dm.TreeModifier.prototype.moveLast = function () {
	...
	if ( step.node instanceof ve.dm.TextNode ) {
		...
		this.remover.node.adjustLength( -step.offsetLength );
		// TODO kill empty text nodes
		this.remover.offset -= step.offsetLength;
		this.inserter.node.adjustLength( step.offsetLength );
		this.inserter.offset += step.offsetLength;
		...
		this.inserter.adjustPath( this.remover.path, step.offset, -1 );
		this.inserter.node.splice( this.inserter.offset, 0, node );
		this.remover.adjustPath( this.inserter.path, this.inserter.offset, 1 );
