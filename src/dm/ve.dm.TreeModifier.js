...
ve.dm.TreeModifier.static.applyTreeOperation = function ( isReversed, document, treeOp ) {
	...
	if ( treeOp.type === 'removeNode' ) {
		...
		node = ve.dm.nodeFactory.createFromElement( treeOp.element );
		if ( node instanceof ve.dm.BranchNode ) {
			node.setupBlockSlugs();
		}
		a.node.splice( a.offset, 0, node );
...
ve.dm.TreeModifier.prototype.processInsert = function ( itemOrData ) {
	...
	if ( type === 'open' ) {
		if ( inserter.node.type === 'text' ) {
			...
			inserter.stepOut();
			// XXX: AAARGGHHH the "even if" above means the linearisation of offsets fails miserably
		}
...
ve.dm.TreeModifier.prototype.getRawPosition = function ( path, offset, node ) {
	...
	if ( nodesBefore ) {
		// We're linearising inside a ContentBranchNode
		for ( i = 0; i < len; i++ ) {
			offset += nodesBefore[ i ].type === 'text' ?
				nodesBefore[ i ].length :
				nodesBefore[ i ].getOuterLength();
		}
	}
