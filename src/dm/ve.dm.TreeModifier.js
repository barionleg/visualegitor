...
ve.dm.TreeModifier.prototype.processRetain = function ( maxLength ) {
	...
	if ( removerStep.type === 'crosstext' ) {
		...
		if ( this.insertedPositions.length ) {
			// XXX why did we have .slice( 0, -1 ) ???
			// this.markAdjustmentNodeComplete( this.getRawRemoverPosition( removerStep ).slice( 0, -1 ) );
			this.markAdjustmentNodeComplete( this.getRawRemoverPosition( removerStep ) );
...
ve.dm.TreeModifier.prototype.pushInsertTextOp = function ( data ) {
	...
	if ( this.insertedPositions.length === 0 ) {
		...
		// We are never inside inserted text
		this.markAdjustmentNodeComplete( rawInserterPosition );
...
ve.dm.TreeModifier.prototype.pushMoveNodeOp = function ( removerStep ) {
	...
	// XXX For all these methods, check the inserter can take the content, using:  
	// - ve.dm.nodeFactory.canNodeHaveChildren
	// - ve.dm.nodeFactory.canNodeHaveChildrenNotContent
	// - ve.dm.nodeFactory.canNodeContainContent
	// on this.insertedPositions.length > 0 ?
	// 	this.insertedPositions[ this.insertedNodes.length - 1 ].type :
...
ve.dm.TreeModifier.prototype.getRawPosition = function ( path, offset, node ) {
	...
	if ( nodesBefore ) {
		for ( i = 0; i < len; i++ ) {
			// XXX should this be the outer node length? 
			offset += nodesBefore[ i ].type === 'text' ? nodesBefore[ i ].length : 2;
		}
	}
...
ve.dm.TreeModifier.prototype.getAdjustedPosition = function ( position, isInserter ) {
	...
	for ( i = 0, iLen = position.length; i < iLen; i++ ) {
		...
		for ( j = 0, jLen = positionI + 1; j < jLen; j++ ) {
			...
			if ( i < iLen - 1 || j < jLen - 1 || !isInserter ) {
