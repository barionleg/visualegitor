 ...
 *
 * Nesting balance is considered lost when a replacement has a non-zero net nesting change (i.e.
 * open tag count minus close tag count) in either its inserted data or its removed data; for
 * instance `[+<p>+][-<h1>-]` causes nesting balance to be lost). Nesting balance is considered
 * restored when the total nesting change since net balance loss is zero for all of: inserted data,
 * removed data, and retained data. For instance,
 * `[+<div>+]<ul><li><div><p>foo</p>[+</div>+]<p>bar</p>[+<div>+]<p>baz</p></div></li></ul>[+</div>+]`
 * is a minimal operation sequence that loses then restores nesting balance. For most plausible
 * operation sequences, this criterion is sufficient to ensure that the multi-splice modification as
 * a whole preserves tree validity of the linear model data; however, it will fail on certain
 * sequences such as `<p>foo[+</p><h1>+]bar[+</h1><p>+]baz</p>`.
 *
