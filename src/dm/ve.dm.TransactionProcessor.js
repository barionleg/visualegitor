 ...
 * of the splices array).
 *
 * @param {Object[]} splices Array of splice operations to execute
 * @param {string} splices[].type 'data' or 'metadata'
 * @param {number} splices[].offset Offset to remove/insert at (unadjusted)
 * @param {number} splices[].removeLength Number of elements to remove
 * @param {Array} splices[].insert Data to insert
...
ve.dm.TransactionProcessor.modifiers.splice = function ( splices, splitAncestorLevel ) {
	var i, s, selection, node, parent, textNode, splitAncestor, spliceOffset, index, numNodes,
	...
	for ( i = 0; i < splices.length; i++ ) {
		...
		s.offset += this.adjustment;
		// Adjust s.offset for previous iterations of this loop (i.e. unsynced modifications);
		// this is the value we need for the actual array splice.
		spliceOffset = s.offset + lengthDiffs[ s.type ];

		// Perform the splice and put the removed data in s, for the undo function
		s.removedData = s.dataObj.batchSplice( spliceOffset, s.removeLength, s.insert );
	...
	if (
		...
		selection = this.document.selectNodes(
			new ve.Range(
				dataSplices[ 0 ].offset,
				dataSplices[ 0 ].offset + dataSplices[ 0 ].removeLength
			),
			'leaves'
		);
	...
	if ( !syncCompleted && dataSplices.length > 0 ) {
		...
		if ( splitAncestorLevel > 0 ) {
			...
			splitAncestor = this.document.getBranchNodeFromOffset( dataSplices[ 0 ].offset );
		...
		// getBranchNodeFromOffset( dataSplices[ 0 ].offset ), and splitAncestor.getOffset() + 1
		// is the lowest value of x such that getBranchNodeFromOffset( x ) returns splitAncestor, so
		// splitAncestor.getOffset() + 1 <= dataSplices[ 0 ].offset,
		// so splitAncestor.getOffset() < dataSplices[ 0 ].offset
		selection = this.document.selectNodes(
			new ve.Range(
				splitAncestor ? splitAncestor.getOffset() : dataSplices[ 0 ].offset,
				dataSplices[ dataSplices.length - 1 ].offset + dataSplices[ dataSplices.length - 1 ].removeLength
			),
			'siblings'
		);
...
ve.dm.TransactionProcessor.modifiers.emitAnnotate = function ( range ) {
	...
	// We didn't modify the linear model, so there are no linmod changes to undo
	return function () {};
};
