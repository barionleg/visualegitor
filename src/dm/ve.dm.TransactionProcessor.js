 ...
 *
 * For available method names, see ve.dm.ElementLinearData
 *
...
/**
 * Splice data into / out of the data array.
 *
 * @param {string} type Always 'data' ('metadata' is no longer used)
...
ve.dm.TransactionProcessor.modifiers.splice = function ( type, offset, remove, insert ) {
	var removed, data,
		isReversed = this.transaction.isReversed,
		doc = this.document;
	if ( type === 'metadata' ) {
		throw new Error( 'Type "metadata" is no longer used' );
	}
	insert = insert || [];
	data = doc.data;
	// TODO: Argh, this selectNodes stuff cannot work because the document
	// is in mid-modification. Instead, just step backwards from offset
	// until we hit a close-CBN (in which case do nothing) or an open-CBN
	// (in which case increment the change counter if it exists). We don't
	// have to handle CBNs inside the removed range because they're disappearing.
	doc.selectNodes(
		new ve.Range( offset, offset + remove ),
		'branches'
	).forEach( function ( selection ) {
		var item = data.data[ selection.nodeOuterRange.start ];
		if ( item.internal && item.internal.changesSinceLoad !== undefined ) {
			item.internal.changesSinceLoad += ( isReversed ? -1 : 1 );
		}
	} );
	...
	return function () {
		...
		doc.selectNodes(
			...
			var internal = data.data[ selection.nodeOuterRange.start ].internal;
			if ( internal && internal.changesSinceLoad !== undefined ) {
				internal.changesSinceLoad += isReversed ? -1 : 1;
			}
		} );
	};
};
