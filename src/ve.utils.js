 ...
 * @param {Function|string} [options.noDescend] jQuery selector: test whether to descend into node
 * @param {Function|string} [options.enterNode]: jQuery selector: test whether to stop walking
 * @param {Function|string} [options.leaveNode]: jQuery Selector: test whether to stop walking
 * @return {Node|null} The node we stopped at (null if we fell off the tree without stopping)
 */
ve.walkDomTree = function ( direction, node, options ) {
	var forward = direction > 0,
		noDescend = options && options.noDescend,
		enterNode = options && options.enterNode,
		leaveNode = options && options.leaveNode;
	...
	while ( true ) {
		...
		while ( true ) {
			if ( enterNode !== undefined && $( node ).is( enterNode ) ) {
				return node;
			} else if (
				node.firstChild &&
				( noDescend === undefined || !$( node ).is( noDescend ) )
			) {
		...
		while ( true ) {
			if ( leaveNode !== undefined && $( node ).is( leaveNode ) ) {
				return node;
			}
