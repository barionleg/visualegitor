 ...
 * search continues to the next matching DOM position. The ve.isHardCursorStep function
 * can be useful here.
 *
 ...
 * @param {string} type One of "enter", "leave", "cross" or (for text nodes) "internal"
 * @param {number} [offset] Defined only for "internal" text node steps
 * @return {boolean} Whether the cursor movement step uses up a cursor press
 * @see ve#adjacentDomPosition
 */
ve.isHardCursorStep = function ( node, type ) {
	if ( node.nodeType === Node.TEXT_NODE ) {
		return type === 'internal';
	}
	return ve.isBlockElement( node ) || ve.rejectsCursor( node );
};
