 ...
 * excluded.
 *
 * As well as the end position, an array of the steps taken is returned. Each step is
 * { node: X, type: Y, offset: Z } . It gives the node stepped, the type of step (which can be
 * "enter", "leave", or "cross" for any node, or "internal" for a step over a character in a
 * text node), and the offset (defined for "internal" steps only).
 *
 * The "stop" option is a boolean-valued function taking ( node, type, offset ) , with which
 * each step is tested. If it returns true, the position arrived at is returned; else the
 * search continues to the next matching DOM position. One useful such function is
 * ve.hasHardCursorBoundaries.
 *
 ...
 * @param {Function} options.stop Boolean-valued test function for stopping positions
 ...
 * @see ve#hasHardCursorBoundaries
 */
 ...
 *
 * Essentially, this is true unless the node is a contentEditable text/annotation node. For
 * instance, in &lt;#text&gt;X&lt;/#text&gt;&lt;b&gt;&lt;#text&gt;y&lt;/#text&gt;&lt;/b&gt; , a
 * single cursor press will jump from just after X to just after Y.
 *
...
ve.hasHardCursorBoundaries = function ( node ) {
	return ve.rejectsCursor( node ) || (
		node.nodeType === node.ELEMENT_NODE &&
		ve.isBlockElement( node )
	);
};
