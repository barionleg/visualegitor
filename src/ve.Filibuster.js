 ...
 * which must be a string, a number, a boolean, undefined or null (to ensure immutability).
 *
 ...
 *
 * @return {ve.Filibuster} Filibuster object, for chaining
 */
...
ve.Filibuster.prototype.observe = function ( action ) {
	...
	function clonePlain( x ) {
		return filibuster.clonePlain( x );
	}
	...
	for ( name in this.observers ) {
		...
		if ( oldState !== newState ) {
			// Write observation, clonePlaining the stack to avoid subsequent
			// anachronistic changes, and for convenient display
			this.observations.push( {
				name: name,
				logCount: this.count,
				oldState: oldState,
				newState: newState,
				stack: this.stack.map( clonePlain ),
				action: action
			} );
 ...
 * @param {string} action The function call phase: call|return|throw
 * @param {Array} data The call arguments, return value or exception
 */
ve.Filibuster.prototype.log = function ( funcName, action, data ) {
	...
	if ( action === 'call' ) {
		this.stack.push( { funcName: funcName, data: data } );
	}
	this.count++;
	this.observe( action );
	// clonePlain data to avoid subsequent anachronistic changes.
	// It would be nice to clonePlain stack too, but it takes a lot of memory.
	this.callLog.push( {
		count: this.count,
		stack: this.stack.slice(),
		funcName: funcName,
		action: action,
		data: this.clonePlain( data )
	} );
	if ( action !== 'call' ) {
...
ve.Filibuster.prototype.wrapNamespace = function ( ns, nsName, blacklist ) {
	...
	for ( i = 0, len = propNames.length; i < len; i++ ) {
		...
		if ( isConstructor ) {
			...
			this.wrapNamespace( ns, nsName + '.' + propName, blacklist );
		}
	}
	return this;
};
 ...
 * @param {Object} [seen] ES6 Set of seen objects, for recursion detection
