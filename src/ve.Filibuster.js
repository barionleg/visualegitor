...
ve.Filibuster.prototype.log = function ( funcName, action, args, returned ) {
	...
	if ( action === 'return' || action === 'throw' ) {
		...
		this.frame.end = time;
		if ( action === 'return' ) {
			this.frame.returned = this.clonePlain( returned );
...
ve.Filibuster.prototype.getObservationsHtml = function ( branchPath ) {
	...
	function showCallClose( frame ) {
		return (
			...
			'--->' +
			ve.escapeHtml(
				frame.thrown ? 'thrown' : String( frame.returned )
			)
		);
	}
	...
	function getFragments( frames, observations, path ) {
		...
		for ( i = 0, iLen = frames.length; i < iLen; i++ ) {
			...
			if ( observation && observation.changes && observation.changes.enter ) {
				html.push( showChanges( observation.changes.enter ) );
			}
			...
			html.push( ' ve-filibuster-frame-' + path.concat( i ).join( '_' ) + '">' );
			if ( expanded ) {
			...
			if ( observation && observation.changes && observation.changes.exit ) {
				html.push( showChanges( observation.changes.exit ) );
			}
		}
		html.push( '</ul>' );
		return html;
	...
	for ( i = 0, iLen = branchPath.length; i < iLen; i++ ) {
		...
		observationTree = observationTree.children[ branchPath[ i ] ] || { children: {} };
	}

	return getFragments(
		callTree.children || [],
		observationTree.children || {},
		branchPath
	).join( '' );
};
