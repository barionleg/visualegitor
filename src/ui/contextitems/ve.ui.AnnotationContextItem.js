...
ve.ui.AnnotationContextItem.prototype.getAnnotationView = function () {
	...
	function filterFn( annotationView ) {
		return model === annotationView.model;
	}

	if ( surfaceView.contexedAnnotations ) {
		annotations = surfaceView.contexedAnnotations.filter( filterFn );
	}
	if ( !annotations.length ) {
		annotations = surfaceView.annotationsAtModelSelection( filterFn );
	}
