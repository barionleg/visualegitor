...
ve.ui.Surface.prototype.startFilibuster = function () {
	...
	if ( !this.filibuster ) {
		...
		this.filibuster.wrapClass( ve.EventSequencer );
		...
		this.filibuster.setObserver( 'dm doc', function () {
			return JSON.stringify( uiSurface.model.documentModel.data.data );
		} );
		this.filibuster.setObserver( 'dm range', function () {
		...
		this.filibuster.setObserver( 'DOM doc', function () {
			return uiSurface.view.$element.html();
		} );
		this.filibuster.setObserver( 'DOM selection', function () {
			var range,
