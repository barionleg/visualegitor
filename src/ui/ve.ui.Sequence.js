...
ve.ui.Sequence.prototype.execute = function ( surface, range ) {
	...
	if ( stripFragment && !executed ) {
		// FIXME Is there a way to undo last transaction without affecting user-visible undo/redo tools?
		// I copied this from WindowAction#open "Sequence-triggered window closed without action, undo",
		// where is has the same issue (you can "redo" after cancelling the dialog to remove the text).
		surfaceModel.undo();
	}
