...
ve.ui.Sequence.prototype.execute = function ( surface, range ) {
	...
	if ( executed && stripFragment ) {
		...
		if ( stripRemainder.length === 1 && stripRemainder[ 0 ].range.isCollapsed() && stripRemainder[ 0 ].node.type === 'paragraph' ) {
			// Stripping the text left us with an empty paragraph node, which we should remove
