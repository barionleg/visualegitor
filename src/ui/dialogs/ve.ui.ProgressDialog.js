...
ve.ui.ProgressDialog.prototype.getSetupProcess = function ( data ) {
	...
	return ve.ui.ProgressDialog.super.prototype.getSetupProcess.call( this, data )
		.next( function () {
			var i, l, progressBar, fieldLayout, cancelDeferred,
			...
			for ( i = 0, l = progresses.length; i < l; i++ ) {
				...
				this.text.$element.append( fieldLayout.$element );
				progresses[i].progressBarDeferred.resolve( progressBar, cancelDeferred.promise() );
				/*jshint loopfunc:true */
				progresses[i].progressCompletePromise.always( this.progressComplete.bind( this ) );
...
ve.ui.ProgressDialog.prototype.progressComplete = function () {
	this.inProgress--;
	if ( !this.inProgress ) {
		this.close();
	}
