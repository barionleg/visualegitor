...
ve.ui.DiffElement.prototype.renderDiff = function ( diff, internalListDiff ) {
	...
	for ( group in internalListDiff.groups ) {
		...
		for ( i = 0, ilen = internalListGroup.length; i < ilen; i++ ) {
			...
			if ( internalListItem.diff === 1 ) {
				...
				internalListDiffQueue.push( [ 'getInternalListNodeElements', internalListDiff, internalListItem, 'insert', null ] );

			} else if ( internalListItem.diff === -1 ) {

				internalListDiffQueue.push( [ 'getInternalListNodeElements', internalListDiff, internalListItem, 'remove', null ] );

			} else if ( internalListItem.diff === 0 ) {

				move = internalListGroup.moves[ i ] === 0 ? null : internalListGroup.moves[ i ];
				internalListDiffQueue.push( [ 'getInternalListNodeElements', internalListDiff, internalListItem, 'none', move ] );
	...
	ilen = Math.max( diff.oldNodes.length, diff.newNodes.length );
	var jlen = ilen;
...
ve.ui.DiffElement.prototype.getChangedDocListData = function ( newListNode, diff ) {
	...
	var ilen = Math.max( diff.oldNodes.length, diff.newNodes.length );
	var jlen = ilen;
 ...
 * @return {HTMLElement[]} Elements (not owned by window.document)
 */
ve.ui.DiffElement.prototype.getInternalListNodeElements = function ( internalListDiff, internalListItem, action, move ) {
