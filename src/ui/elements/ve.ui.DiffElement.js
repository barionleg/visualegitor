	...
	for ( group in internalListDiff.groups ) {
		...
		for ( i = 0, ilen = internalListGroup.length; i < ilen; i++ ) {
			...
			if ( internalListItem.diff === 1 ) {
				...
				internalListDiffQueue.push( [ 'getInternalListNodeElements', internalListDiff, internalListItem, 'none', listMove ] );
				...
				if ( cell instanceof ve.dm.TableMatrixCell ) {
					...
					if ( !cell.after.isPlaceholder() ) {
						if ( cell.diff ) {
							ve.batchPush(
								diffData,
								diffElement.getChangedNodeData( cell.diff, cell.before.node, cell.after.node )
							);
						} else {
							// Remove-insert
