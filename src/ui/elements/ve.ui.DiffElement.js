...
ve.ui.DiffElement.prototype.getChangedListNodeData = function ( oldNode, newNode, diff ) {
	var i, ilen, item, depth, newDepth, action, nodes,
		insertIndex, listNode, listItemData, doc,
	...
	function appendListItem( diffData, insertIndex, listNode, listNodeData, listItemData, depthChange ) {
		...
		if ( depthChange === 0 ) {
			...
			// Current list item belongs to the same list as the prevous list item
	...
	for ( i = 0, ilen = diff.length; i < ilen; i++ ) {
		...
		// Get the data for the list item and splice in the data for the content
		listItemData = doc.getData( nodes.metadata[ item.indexOrder ].listItem.getOuterRange() );
		if ( item.diff.attributeChange && item.diff.attributeChange.listItemAttributeChange ) {
		...
		// Pass in the list node itself rather than the linear data, since may
		// need to access its ancestors
		listNode = nodes.metadata[ item.indexOrder ].listNode;
		// TODO: Not necessary unless depthChange > 0
		listNodeData = [ doc.getData( listNode.getOuterRange() )[ 0 ] ];
		if ( item.diff.attributeChange && item.diff.attributeChange.listNodeAttributeChange ) {
			item = this.compareNodeAttributes(
				listNodeData,
				0,
				this.newDoc,
		...
		insertIndex = appendListItem.call( this, diffData, insertIndex, listNode, listNodeData, listItemData, depthChange );
		depth = newDepth;
	}
...
ve.ui.DiffElement.prototype.compareNodeAttributes = function ( data, offset, doc, attributeChange ) {
	...
	if ( changes.length && ( !( data[ offset ].internal.diff ) || data[ offset ].internal.diff[ 'data-diff-id' ] === undefined ) ) {
