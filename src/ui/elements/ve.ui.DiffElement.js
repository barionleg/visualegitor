...
ve.ui.DiffElement.prototype.getChangedListNodeData = function ( oldNode, newNode, diff ) {
	var i, ilen, item, depth, newDepth, action, nodes,
		...
		depthChange, contentData,
		nodeRange = newNode.getOuterRange(),
		diffData = this.newDoc.getData( nodeRange ),
		oldNodes = diff.oldList,
		newNodes = diff.newList;

	// insertIndex is index of where next item should be spliced in, if depth change is 0
	function appendListItem( diffData, insertIndex, listNode, listItemData, depthChange ) {
	...
	for ( i = 0, ilen = diff.length; i < ilen; i++ ) {
		...
		insertIndex = appendListItem.call( this, diffData, insertIndex, listNode, listItemData, depthChange );
		depth = newDepth;
	}
