...
ve.ui.DiffElement.prototype.getChangedNodeElements = function ( oldNodeIndex, move ) {
	...
	function highlightChangedSubTree( nodeIndex ) {
		...
		if ( subTreeDiffInfo.attributeChange ) {
			this.compareAttributes( nodeData, subTreeRootNodeRangeStart, this.newDoc, subTreeDiffInfo.attributeChange );
		}
	}
...
ve.ui.DiffElement.prototype.compareAttributes = function ( data, offset, doc, attributeChange ) {
	var i, l, changes, key, oldAttributes, newAttributes,
		$label = $( [] ),
		element = data[ offset ],
		attributeChanges = {};
	...
	oldAttributes = attributeChange.oldAttributes;
	newAttributes = attributeChange.newAttributes;
	...
	data[ offset ] = this.addAttributesToNode( element, doc, { 'data-diff-id': this.elementId } );

	changes = ve.dm.modelRegistry.lookup( element.type ).static.describeChanges( attributeChanges, newAttributes );
