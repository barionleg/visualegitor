...
ve.ui.DiffElement.prototype.compareAttributes = function ( node, attributeChange ) {
	var changes, key, oldAttributes, newAttributes,
		diffElement = this,
		attributeChanges = {};
	...
	changes = node.constructor.static.describeChanges( attributeChanges, newAttributes );

	changes.forEach( function ( change ) {
		diffElement.$sidebar.append(
			new ve.ui.AttributeChangeElement( change ).$element
		);
	} );
};
 ...
 *
 * If the node already has the attribute it is appended to the current value
 *
