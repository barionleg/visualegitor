...
ve.ui.DiffElement.prototype.getChangedTableNodeData = function ( oldNode, newNode, diff ) {
	var colInfo, lastRowNode, lastSectionNode,
		diffElement = this,
	...
	colInfo = diff.tableDiff.data[ 0 ].slice( 1 );
	diff.tableDiff.data.slice( 1 ).forEach( function ( row, rowIndex ) {
		var rowNode, sectionNode,
			rowInfo = row[ 0 ];

		rowNode = ( row[ 1 ].after || row[ 1 ] ).owner.node.parent;
		sectionNode = rowNode.parent;
		...
		if ( rowInfo === '+++' || rowInfo === '---' ) {
			...
			row.slice( 1 ).forEach( function ( cell, colIndex ) {
				var cellData, beforeCellData, afterCellData,
					beforeCellDiv, afterCellDiv,
					move, item, attrs;
				...
				if ( cell instanceof ve.dm.TableMatrixCell ) {
					if ( !cell.isPlaceholder() ) {
						...
						if ( colInfo[ colIndex ] === '+++' ) {
							...
							attrs = { 'data-diff-move': move };
							if ( move ) {
								// The following messages are used here:
								// * visualeditor-diff-moved-table-col
								// * visualeditor-diff-moved-table-row
								item = diffElement.getChangeDescriptionItem( [ ve.msg( 'visualeditor-diff-moved-' + move ) ] );
					...
					if ( !cell.after.isPlaceholder() ) {
						if ( cell.diff ) {
							...
							// Remove-insert
							afterCellData = getData( cell.after.node );
							afterCellDiv = { type: 'div' };
							diffElement.addAttributesToElement( afterCellDiv, { 'data-diff-action': 'insert' } );
							beforeCellData = getData( cell.before.node );
							beforeCellDiv = { type: 'div' };
							diffElement.addAttributesToElement( beforeCellDiv, { 'data-diff-action': 'remove' } );
