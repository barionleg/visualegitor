 ...
 *
 * @constructor
 * @param {ve.dm.VisualDiff} [diff] Diff to visualize
 */
ve.ui.DiffElement = function VeUiDiffElement( visualDiff, oldDoc, newDoc ) {
	...
	// This will be an array of the html diff for each doc child node:
	this.diffHtml = this.getDiffHtml();

	// TODO: join the array of html into a diff
};
...
ve.ui.DiffElement.prototype.getDiffHtml = function () {
	...
	for ( i = 0, j = 0; i < ilen, j < jlen; i++, j++ ) {
		if ( this.oldDocChildren[ i ] === undefined ) {
			...
			diffHtml.push( this.getChangedNodeHtml( i, move ) );
...
ve.ui.DiffElement.prototype.getNodeHtml = function ( node, action, move ) {
	var nodeData, nodeDoc, nodeHtml, classes = [];

	if ( action ) {
		classes.push( 've-ui-DiffElement-diff-' + action );
	}
	if ( move ) {
		classes.push( 've-ui-DiffElement-move-' + move );
	}
	...
	nodeData[ 0 ] = this.addClassesToNode( nodeData[ 0 ], nodeDoc, classes );
...
ve.ui.DiffElement.prototype.getChangedNodeHtml = function ( oldNodeIndex, move ) {
	...
	function nodesInsertOrRemove( nodeIndex, action ) {
		var i, ilen,
			doc, nodes,
			subNodeTreeNode,
			subNode,
		...
		subNodeData[ 0 ] = this.addClassesToNode( subNodeData[ 0 ], doc, [ 've-ui-DiffElement-diff-' + action ] );
		...
		// Find node's descendants and modify them
	...
	for ( i = 0, j = 0; i < ilen && j < jlen; i++, j++ ) {
		...
		if ( iModified < 0 ) {
			...
			if ( alreadyProcessed.remove.indexOf( jModified ) === -1 ) {
				nodesInsertOrRemove.call( this, jModified, 'remove' );
			}
			...
			if ( alreadyProcessed.insert.indexOf( iModified ) === -1 ) {
				nodesInsertOrRemove.call( this, iModified, 'insert' );
			}
			...
			if ( markedNodes.insert.indexOf( iModified ) !== -1 ) {
				...
				for ( k = 0, klen = treeDiff.length; k < klen; k++ ) {
					if ( treeDiff[ k ][ 0 ] === iModified && treeDiff[ k ][ 1 ] === jModified ) {
						nodesInsertOrRemove.call( this, iModified, 'change' );
					}
				}
			}
...
ve.ui.DiffElement.prototype.addClassesToNode = function ( nodeData, nodeDoc, classes ) {
	...
	for ( i = 0; i < ilen; i++ ) {
		...
		if ( start !== end ) {
			...
			if ( linearDiff[ i ][ 0 ] === 1 || linearDiff[ i ][ 0 ] === -1 ) {
				diffClass = 've-ui-DiffElement-diff-' + ( linearDiff[ i ][ 0 ] === 1 ? 'insert' : 'remove' );
				domElement = document.createElement( 'span' );
