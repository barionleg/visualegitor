...
ve.ui.DebugBar.prototype.testSquasher = function () {
	var i, iLen, squashed, squashedBefore, squashedAfter, doubleSquashed,
		transactions = this.getSurface().getModel().getDocument().completeHistory;
	if ( transactions.length < 3 ) {
		// Nothing interesting here
		return;
	}
	// Freeze the transactions, to help detect any mistaken reference copying. If you do
	// this, you will probably want to put "use strict"; at the top of any suspect file so
	// assignments throw errors instead of failing silently
	// transactions.forEach( function ( tx ) {
	// 	tx.debugFreeze();
	// } );
	squashed = ve.dm.TransactionSquasher.static.squash( transactions ).serialize();
	for ( i = 1, iLen = transactions.length - 1; i < iLen; i++ ) {
		squashedBefore = ve.dm.TransactionSquasher.static.squash( transactions.slice( 0, i ) );
		squashedAfter = ve.dm.TransactionSquasher.static.squash( transactions.slice( i ) );
		doubleSquashed = ve.dm.TransactionSquasher.static.squash( [
			squashedBefore,
			squashedAfter
		] ).serialize();
		if ( JSON.stringify( squashed ) !== JSON.stringify( doubleSquashed ) ) {
