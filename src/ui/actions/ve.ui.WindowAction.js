	...
	fragmentPromise.then( function ( fragment ) {
		...
		ve.promiseAll( autoClosePromises ).always( function () {
			windowManager.getWindow( name ).then( function ( win ) {
				...
				if ( !win.constructor.static.activeSurface ) {
					// Use the closing promise to wait until the dialog has performed its actions,
					// such as creating new annotations, before re-activating.
					instance.closing.then( function () {
						// Don't activate if mobile and expanded. We only need to re-activate the surface
						// if an insertion annotation was generated, which implies a collapsed selection.
						if ( !OO.ui.isMobile() || surface.getModel().getSelection().isCollapsed() ) {
							surface.getView().activate();
						}
					} );

					// On mobile, we need to re-activate the surface during the same event cycle, otherwise
					// the device may not open the virtual keyboard, so duplicate our handling for the mobile
					// case in the 'closing' event. (T203517)
					if ( OO.ui.isMobile() ) {
						windowManager.once( 'closing', function () {
							if ( surface.getModel().getSelection().isCollapsed() ) {
								surface.getView().activate();
							}
						} );
					}
