...
ve.ui.TableAction.prototype.importTable = function ( importedTableNode, importInternalList ) {
	...
	for ( row = importedMatrix.getRowCount() - 1; row >= 0; row-- ) {
		for ( col = importedMatrix.getColCount( row ) - 1; col >= 0; col-- ) {
			...
			if ( !( importedCell.node instanceof ve.dm.TableCellNode ) ) {
				// Since the imported cell isn't a regular TableCellNode, we
				// can't quite trust our assumptions about how it's supposed
				// to work. As such, it's safer to outright replace the cell
				// rather than trying to be clever and switch out the
				// attributes / data. We shouldn't have gotten to this point
				// without it being Cellable, so this should at least work.
