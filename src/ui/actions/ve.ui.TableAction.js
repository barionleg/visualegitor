 ...
 * @property
 */
ve.ui.TableAction.static.methods = [ 'create', 'insert', 'delete', 'move', 'moveRelative', 'changeCellStyle', 'mergeCells', 'caption', 'enterTableCell' ];
...
ve.ui.TableAction.prototype.moveRelative = function ( mode, direction ) {
	...
	if ( mode === 'row' ) {
		...
		if ( direction === 'before' ) {
			...
			index = Math.min( matrix.getColCount(), selection.endCol + 2 );
		}
	}
...
ve.ui.TableAction.prototype.move = function ( mode, index ) {
	var i, l, removedMatrix, position, newSelection,
		surfaceModel = this.surface.getModel(),
		selection = surfaceModel.getSelection(),
