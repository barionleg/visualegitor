...
ve.ui.TableAction.prototype.importTable = function ( importedTableNode ) {
	var i, l, row, col, cell, importedCell,
		txs = [],
	...
	for ( i = 0, l = selection.fromRow + importedMatrix.getRowCount() - matrix.getRowCount(); i < l; i++ ) {
		this.insertRowOrCol( tableNode, 'row', matrix.getRowCount() - 1, 'after' );
	}
	for ( i = 0, l = selection.fromCol + importedMatrix.getColCount() - matrix.getColCount(); i < l; i++ ) {
		this.insertRowOrCol( tableNode, 'col', matrix.getColCount() - 1, 'after' );
	}
	// Unmerge all cells in the target area
	for ( row = importedMatrix.getRowCount() - 1; row >= 0; row-- ) {
		for ( col = importedMatrix.getColCount() - 1; col >= 0; col-- ) {
	...
	for ( row = importedMatrix.getRowCount() - 1; row >= 0; row-- ) {
		for ( col = importedMatrix.getColCount() - 1; col >= 0; col-- ) {
			...
			txs.push( ve.dm.Transaction.newFromRemoval( documentModel, cell.node.getOuterRange() ) );
			if ( !importedCell.isPlaceholder() ) {
				txs.push(
					ve.dm.Transaction.newFromDocumentInsertion(
						documentModel, cell.node.getOuterRange().start,
						importedTableNode.getDocument(),
						importedCell.node.getOuterRange()
					)
				);
			}
		}
