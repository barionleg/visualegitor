...
ve.ui.TableAction.prototype.create = function ( options ) {
	...
	if ( options.header ) {
		data.push( ...ve.dm.TableRowNode.static.createData( { style: 'header', cellCount: numberOfCols } ) );
	}
	for ( let i = 0; i < numberOfRows; i++ ) {
		data.push( ...ve.dm.TableRowNode.static.createData( { style: 'data', cellCount: numberOfCols } ) );
	}
		...
		if ( !dataMatrixLine ) {
			...
			insertCells.forEach( ( c ) => {
				if ( c && c.data ) {
					...
					insertData.push( ...ve.dm.TableCellNode.static.createData() );
				}
			} );
...
ve.ui.TableAction.prototype.deleteRowsOrColumns = function ( matrix, mode, minIndex, maxIndex ) {
	...
	if ( mode === 'row' ) {
		for ( let row = minIndex; row <= maxIndex; row++ ) {
			cells.push( ...matrix.getRow( row ) );
		}
	} else {
		for ( let col = minIndex; col <= maxIndex; col++ ) {
			cells.push( ...matrix.getColumn( col ) );
		}
	}
