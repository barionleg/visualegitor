 ...
 * @property
 */
ve.ui.TableAction.static.methods = [ 'create', 'insert', 'delete', 'move', 'changeCellStyle', 'mergeCells', 'caption', 'enterTableCell' ];
...
ve.ui.TableAction.prototype.move = function ( mode, index ) {
	var i, l, removedMatrix, position,
		surfaceModel = this.surface.getModel(),
		selection = surfaceModel.getSelection(),
	...
	if ( mode === 'row' ) {
		removedMatrix = this.deleteRowsOrColumns( matrix, mode, selection.fromRow, selection.toRow );
		if ( index > selection.toRow ) {
			index = index - selection.getRowCount();
		}
	} else {
		removedMatrix = this.deleteRowsOrColumns( matrix, mode, selection.fromCol, selection.toCol );
		if ( index > selection.toCol ) {
	...
	for ( i = 0, l = removedMatrix.length; i < l; i++ ) {
		this.insertRowOrCol( tableNode, mode, index, position, selection, removedMatrix[ i ] );
	}
	return true;
};
	...
	for ( i = 0, l = cells.length; i < l; i++ ) {
		...
		if ( refCell && ( cell.isPlaceholder() || refCell.isPlaceholder() ) ) {
			if ( cell.node === refCell.node ) {
				...
				if (
					dataMatrixLine && dataMatrixLine.cells[ i ].isPlaceholder() &&
					dataMatrixLine.cells[ i ][ mode ] === dataMatrixLine.cells[ i ].owner[ mode ]
				) {
					// If we skipped inserting a placeholder, decrement the span of the owner
					if ( mode === 'row' ) {
						dataMatrixLine.cells[ i ].owner.data.attributes.colspan -= 1;
					} else {
						dataMatrixLine.cells[ i ].owner.data.attributes.rowspan -= 1;
					}
				}
				continue;
			}
	...
	if ( mode === 'row' ) {
		if ( !dataMatrixLine ) {
			...
			insertCells.forEach( function ( cell ) {
				insertData = insertData.concat( cell.data || ve.dm.TableCellNode.static.createData() );
			} );
		...
		for ( i = 0; i < inserts.length; i++ ) {
			...
			if ( !dataMatrixLine ) {
				...
				cellData = dataMatrixLine.cells[ cell.row ].data;
			}
	...
	if ( mode === 'row' ) {
			...
			removedMatrix[ row - minIndex ] = {
				...
				cells: cells.map( function ( cell ) {
					if ( !cell.isPlaceholder() ) {
						...
						cell.data[ 0 ].attributes.rowspan = 1;
					} else if ( cell.owner.row === cell.row ) {
						// If placeholder for same row, data to re-insert is empty,
						// otherwise data will be undefined and a new blank cell will
						// be generated
						cell.data = [];
					}
					return cell;
		...
		for ( i = 0; i < actions.length; i++ ) {
			if ( actions[ i ].action === 'insert' ) {
				...
				removedMatrix[ col ] = removedMatrix[ col ] || {
					cells: matrix.getColumn( actions[ i ].cell.col ).map( function ( cell ) {
						if ( !cell.isPlaceholder() ) {
							cell.data = documentModel.getData( cell.node.getOuterRange(), true );
							cell.data[ 0 ].attributes.colspan = 1;
						} else if ( cell.owner.col === cell.col ) {
							// If placeholder for same column, data to re-insert is empty,
							// otherwise data will be undefined and a new blank cell will
							// be generated
							cell.data = [];
						}
