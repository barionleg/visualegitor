 ...
 * @abstract
 * @param {string} input
 * @param {int} [limit=20] restrict number of results to this
 ...
 *
 * @param {string} input
 * @param {int} matches Number of matches before the input occurred
 * @return {boolean} Whether to abandon
 */
ve.ui.CompletionAction.prototype.shouldAbandon = function ( input, matches ) {
	// abandon after pressing space if there are no active potential matches
	return matches === ( this.constructor.static.alwaysIncludeInput ? 1 : 0 ) && input[ input.length - 1 ] === ' ';
};
