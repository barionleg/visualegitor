...
[ 'extension.json', 'skin.json' ].forEach( ( file ) => {
	...
	if ( fs.existsSync( extensionPath ) ) {
		...
		if ( extension.namemsg ) {
			extensionMessages.push( extension.namemsg );
		}
		if ( extension.descriptionmsg ) {
			extensionMessages.push( extension.descriptionmsg );
		}
		if ( extension.AvailableRights ) {
			extension.AvailableRights.forEach( ( right ) => {
				extensionMessages.push( 'right-' + right );
				extensionMessages.push( 'action-' + right );
			} );
		}
		if ( extension.GrantPermissions ) {
			extensionMessages.push.apply( extensionMessages, Object.keys( extension.GrantPermissions ).map(
				( right ) => 'grant-' + right
			) );
		}
		if ( extension.SpecialPages ) {
			extensionMessages.push.apply( extensionMessages, Object.keys( extension.SpecialPages ).map(
				( specialPage ) => specialPage.toLowerCase()
			) );
		}
		if ( extension.LogTypes ) {
			extensionMessages.push.apply( extensionMessages, extension.LogTypes.map(
				( type ) => 'log-name-' + type
			) );
			extensionMessages.push.apply( extensionMessages, extension.LogTypes.map(
				( type ) => 'log-description-' + type
			) );
			extensionMessages.push.apply( extensionMessages, extension.LogTypes.map(
				( type ) => 'logeventslist-' + type + '-log'
			) );
		}
		if ( extension.LogNames ) {
			extensionMessages.push.apply( extensionMessages, Object.values( extension.LogNames ) );
		}
		if ( extension.LogHeaders ) {
			extensionMessages.push.apply( extensionMessages, Object.values( extension.LogHeaders ) );
		}
		if ( extension.ActionFilteredLogs ) {
			Object.keys( extension.ActionFilteredLogs ).forEach( ( type ) => {
				extensionMessages.push( 'log-action-filter-' + type );
				extensionMessages.push.apply( extensionMessages, Object.values( extension.ActionFilteredLogs[ type ] ).map(
					( action ) => 'log-action-filter-' + type + '-' + action
				) );
				extensionMessages.push.apply( extensionMessages, Object.values( extension.ActionFilteredLogs[ type ] ).map(
					( action ) => 'logentry-' + type + '-' + action
				) );
			} );
		}
	}
...
function findKeysInSourceCode( fileContent ) {
	...
	const messageKeysPattern = new RegExp( '(?:^|[^a-z-_])(' + messageKeys.join( '|' ) + ')(?:$|[^a-z-_])', 'g' );
...
checkContents( extensionMessages.join( '  ' ) );
