...
function setup ( elts ) {
	...
	var fixAnnotation = function () {
		...
		if ( bNode.childNodes.length === 0 && bNode.parentNode ) {
	...
	elts.content.addEventListener( 'input', refresh );
	elts.content.addEventListener( 'keyup', refresh );
	elts.html.addEventListener( 'input', function () {
		elts.content.innerHTML = elts.html.textContent;
	} );
	repeatShowParts = function () {
		showParts();
		document.title = new Date();
		setTimeout( repeatShowParts, 1000 );
	};
	repeatShowParts();
}
