...
function* welcomeNewClient( context ) {
	var state, authorData;
	yield rebaseServer.updateDocState( context.docName, context.author, null, {
		displayName: 'User ' + context.author // TODO: i18n
	} );

	state = yield rebaseServer.getDocState( context.docName );
	authorData = state.authors.get( context.author );
