	...
	return function handleConnection( socket ) {
		...
		if ( existingState ) {
			...
			socket.emit( 'newUpdate', {
				author: null,
				change: existingState.change.serialize(),
		...
		socket.on( 'submitUpdate', setTimeout.bind( null, function ( data ) {
			...
			try {
				console.log( 'submitUpdate data', data );
				change = ve.dm.Change.static.deserialize( data.change, true );
				selection = ve.dm.Selection.static.newFromJSON( null, data.selection );
				console.log( data.author + ' recv ' + summarize( change ) + ', ' + selection.getDescription() );
				applied = rebaser.applyChange( docName, data.author, change, selection );
				if ( applied ) {
					console.log( data.author + ' applied ' + summarize( applied.change ) + ', ' + ( applied.selection ? applied.selection.getDescription() : null ) );
					docNamespaces.get( docName ).emit( 'newUpdate', {
						author: data.author,
						change: applied.change.serialize(),
