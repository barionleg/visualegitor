		...
		socket.on( 'submitUpdate', setTimeout.bind( null, function ( data ) {
			...
			try {
				if ( data.transactions && data.transactions.length > 0 ) {
					msg.push( data.transactions.length + ' transactions (start=' + data.transactionStart + ')' );
				}
				if ( data.store && data.store.hashes.length > 0 ) {
					msg.push( data.store.hashes.length + ' stored values (start=' + data.storeStart + ')' );
				}
