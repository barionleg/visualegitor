	...
	keys.forEach( function ( keyString ) {
		promise = promise.then( function () {
			...
			if ( !keyDownEvent.isDefaultPrevented() ) {
				...
				view.eventSequencer.onEvent( 'keypress', ve.test.utils.TestEvent( { type: 'keypress' }, keyData ) );
			}
			...
			view.eventSequencer.onEvent( 'keyup', ve.test.utils.TestEvent( { type: 'keyup' }, keyData ) );
...
ve.test.utils.runSurfacePasteTest = function ( assert, htmlOrView, pasteData, internalSourceRangeOrSelection, noClipboardData, fromVe, useClipboardData, pasteTargetHtml, rangeOrSelection, pasteSpecial, expectedOps, expectedRangeOrSelection, expectedHtml, expectedDefaultPrevented, store, msg ) {
	...
	if ( internalSourceRangeOrSelection ) {
		...
		testEvent = ve.test.utils.TestEvent( { type: 'copy', clipboardData: clipboardData } );
		if ( noClipboardData ) {
		...
		testEvent = ve.test.utils.TestEvent( { type: 'paste', clipboardData: clipboardData } );
	} else {
		if ( useClipboardData ) {
 ...
 */
ve.test.utils.TestEvent = function TestEvent( src, props ) {
	var event;
 ...
 *
 * @param {Object} [initialData] Initial data to be passed to setData
 */
