...
ve.test.utils.runSurfacePasteTest = function ( assert, item ) {
	...
	if ( item.internalSourceRangeOrSelection ) {
		...
		view.pasteHandler.onCopy( testEvent );
		if ( item.noClipboardData ) {
		...
		if ( item.useClipboardData ) {
			clipboardData.setData( view.pasteHandler.constructor.static.clipboardKeyMimeType, 'useClipboardData-0' );
		} else if ( item.fromVe ) {
			clipboardData.setData( view.pasteHandler.constructor.static.clipboardKeyMimeType, '0.123-0' );
		}
		testEvent = ve.test.utils.createTestEvent( { type: 'paste', clipboardData: clipboardData } );
	}
	...
	if ( item.pasteSpecial ) {
		view.pasteHandler.prepareForPasteSpecial();
	}

	// Replicate the sequencing of ce.Surface.onPaste, without any setTimeouts:
	view.pasteHandler.beforePaste( testEvent );
	if ( !testEvent.isDefaultPrevented() ) {
		if ( item.pasteHandlerHtml ) {
			view.pasteHandler.$element.html( item.pasteHandlerHtml );
		...
		afterPastePromise = view.pasteHandler.afterPaste( testEvent );
	}
...
QUnit.test( 'getClipboardHash', ( assert ) => {
	assert.strictEqual(
		ve.ce.PasteHandler.static.getClipboardHash(
...
QUnit.test( 'onCopy', ( assert ) => {
	...
	function testRunner( doc, rangeOrSelection, expectedData, expectedOriginalRange, expectedBalancedRange, expectedHtml, expectedText, noClipboardData, msg ) {
		...
		const slice = view.pasteHandler.clipboard.slice;
		const clipboardKey = view.pasteHandler.clipboardId + '-' + view.pasteHandler.clipboardIndex;
		...
		if ( !noClipboardData ) {
			assert.strictEqual( clipboardData.getData( view.pasteHandler.constructor.static.clipboardKeyMimeType ), clipboardKey, msg + ': clipboardId set' );
		}
	...
	const docLen = 30,
		...
		cases = [
			...
			},
			{
				...
				pasteHandlerHtml: $( ve.dm.example.blockImage.html ).unwrap().html(),
				expectedOps: [
					[
						{
							type: 'replace',
			...
			},
			{
				...
				pasteHandlerHtml: '<span data-ve-clipboard-key="0.13811087369534492-4">&nbsp;</span><s>Alien</s>',
				fromVe: true,
				expectedOps: [
					[
						{ type: 'retain', length: 1 },
			...
			},
			{
				...
				pasteHandlerHtml: '<table><tbody><tr><td>X</td><td>Y</td><td>Z</td></tr></tbody></table>',
				fromVe: true,
				expectedRangeOrSelection: {
					type: 'table',
			...
			},
			{
				rangeOrSelection: new ve.Range( 0 ),
				// Firefox doesn't like using execCommand for this test for some reason
				pasteHandlerHtml: '<ul><li>A</li><ul><li>B</li></ul></ul>',
				expectedRangeOrSelection: new ve.Range( 14 ),
				expectedOps: [
					[
						{
			...
			},
			{
				...
				pasteHandlerHtml: 'A<ul><ul><li>B</li></ul></ul>C',
			...
			{
				rangeOrSelection: new ve.Range( 0 ),
				pasteHandlerHtml: '<li>B</li><li>C</li>',
			...
			{
				rangeOrSelection: new ve.Range( 0 ),
				pasteHandlerHtml: '<p>A</p><p></p><p>B</p>',
				expectedOps: [
					[
						{
							type: 'replace',
			...
			{
				rangeOrSelection: new ve.Range( 1 ),
				pasteHandlerHtml: '<img src="' + ve.ce.minImgDataUri + '" width="10" height="20" />',
				expectedOps: [
					[
						{
							type: 'retain',
