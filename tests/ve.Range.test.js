...
QUnit.test( 'Basic usage (clone, isCollapsed, isBackwards, getLength, equals, equalsSelection, containsOffset, containsRange)', 25, function ( assert ) {
	...
	assert.strictEqual( range.containsOffset( 99 ), false, 'doesn\'t contain 50' );
	assert.strictEqual( range.containsOffset( 100 ), true, 'contains 150' );
	assert.strictEqual( range.containsOffset( 199 ), true, 'contains 100' );
	...
	assert.strictEqual( range.containsOffset( 100 ), true, 'contains 150' );
	assert.strictEqual( range.containsOffset( 199 ), true, 'contains 100' );
	assert.strictEqual( range.containsOffset( 200 ), false, 'doesn\'t contain 250' );
	assert.strictEqual( range.containsRange( new ve.Range( 99, 100 ) ), false, 'doesn\'t contain 99, 100' );
	assert.strictEqual( range.containsRange( new ve.Range( 100, 199 ) ), true, 'contains 100, 199' );
