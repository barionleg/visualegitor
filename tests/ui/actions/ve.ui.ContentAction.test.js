...
function runContentActionTest( assert, html, fullSurface, method, args, rangeOrSelection, expectedData, expectedRangeOrSelection, msg ) {
	var surface = fullSurface ?
			ve.test.utils.createSurfaceFromHtml( html || ve.dm.example.html ) :
...
QUnit.test( 'insert', function ( assert ) {
	...
	QUnit.expect( expected );	for ( i = 0; i < cases.length; i++ ) {
		runContentActionTest(
			assert, cases[ i ].html, cases[ i ].fullSurface, cases[ i ].method, cases[ i ].args, cases[ i ].rangeOrSelection,
