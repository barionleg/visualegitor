	...
	var i, len, visualDiff, diffElement,
		...
		cases = [
			...
			},
			{
				...
				expected:
					'<div class="ve-ui-diffElement-doc-child-change">' +
						'<p>foo <span data-diff-action="change-remove"><a href="quuz" target="_blank">bar</a></span><span data-diff-action="change-insert"><a href="whee" target="_blank">bar</a></span> baz</p>' +
					'</div>'
			},
			{
...
QUnit.test( 'describeChange', function ( assert ) {
		...
		cases = [
			{
				...
				testedClass: ve.dm.LinkAnnotation.static,
				testedKey: 'foo',
			...
			},
			{
				...
				testedClass: ve.dm.LanguageAnnotation.static,
				testedKey: 'lang',
	...
	for ( i = 0, l = cases.length; i < l; i++ ) {
		...
		assert.equal(
			cases[ i ].testedClass.describeChange(
				key,
