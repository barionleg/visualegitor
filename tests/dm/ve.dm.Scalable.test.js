...
QUnit.test( 'construction/clone/getters/setters/toggleDefault/clearers', 26, function ( assert ) {
	...
	assert.strictEqual( scalable.getRatio(), 1, 'setOriginalDimensions overwrites the ratio if set' );
...
QUnit.test( 'isDefault/toggleDefault', 4, function ( assert ) {
	...
	scalable.toggleDefault();
	assert.strictEqual( scalable.isDefault(), false, 'isDefault after toggleDefault' );
	scalable.toggleDefault();
	assert.strictEqual( scalable.isDefault(), true, 'isDefault after double toggleDefault' );
} );
