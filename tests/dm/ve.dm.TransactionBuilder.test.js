...
function runBuilderTests( assert, cases ) {
	...
	for ( msg in cases ) {
		...
		assert.deepEqualWithDomElements( tx.getTransaction().getOperations(), cases[ msg ].ops, msg + ': operations match' );
	}
}
...
QUnit.test( 'operations/build from operations', function ( assert ) {
	...
	for ( i = 0; i < cases.length; i++ ) {
		...
		tx = ve.dm.TransactionBuilder.static[ cases[ i ].method ].apply( null, cases[ i ].args );
		ops = ve.copy( tx.operations );
		assert.deepEqual( ops, cases[ i ].expected, cases[ i ].msg + ': operations' );
		if ( cases[ i ].roundTripArgs ) {
			tx = ve.dm.TransactionBuilder.static[ cases[ i ].method ].apply( null, cases[ i ].roundTripArgs );
		}
