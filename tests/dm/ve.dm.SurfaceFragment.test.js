...
QUnit.test( 'getAnnotations', 3, function ( assert ) {
	var tableSelection,
		...
		tableRange = new ve.Range( 0, 27 ),
		surface = new ve.dm.Surface( doc );

	tableSelection = new ve.dm.TableSelection( doc, tableRange, 0, 0, 1, 0 );

	assert.deepEqual( surface.getFragment( tableSelection ).getAnnotations().getIndexes(), [ 0, 1 ], 'Comparable annotations spanning multiple ranges' );

	tableSelection = new ve.dm.TableSelection( doc, tableRange, 0, 0, 2, 0 );
	assert.deepEqual( surface.getFragment( tableSelection ).getAnnotations().getIndexes(), [], 'Non-comparable annotations spanning multiple ranges' );
