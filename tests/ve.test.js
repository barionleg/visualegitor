...
QUnit.test( 'resolveUrl', function ( assert ) {
	...
	for ( i = 0; i < cases.length; i++ ) {
		...
		doc.head.appendChild( $( '<base>' ).attr( 'href', cases[i].base )[0] );
...
QUnit.test( 'fixBase', function ( assert ) {
	...
	for ( i = 0; i < cases.length; i++ ) {
		...
		if ( cases[i].targetBase ) {
			targetDoc.head.appendChild( $( '<base>' ).attr( 'href', cases[i].targetBase )[0] );
		}
		if ( cases[i].sourceBase ) {
			sourceDoc.head.appendChild( $( '<base>' ).attr( 'href', cases[i].sourceBase )[0] );
		}
