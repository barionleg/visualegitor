...
/**
 * Get list of action widgets.
 *
 * The widgets may be visually presented in a different order than they appear in the array.
 *
 * @return {OO.ui.ActionButtonWidget[]}
 */
 ...
 */
OO.ui.ActionDialog.prototype.getActionProcess = function () {
	return new OO.ui.Process();
};
	...
	return OO.ui.ActionDialog.super.prototype.getSetupProcess.call( this, data )
		.next( function () {
			...
			for ( i = 0, len = actions.length; i < len; i++ ) {
				button = new OO.ui.ActionButtonWidget( $.extend( { '$': this.$ }, actions[i] ) );
				if ( !this.safeActionButton && button.hasFlag( 'safe' ) ) {
					this.safeActionButton = button;
				} else if ( !this.primaryActionButton && button.hasFlag( 'primary' ) ) {
					this.primaryActionButton = button;
				} else {
					this.otherActionButtons.push( button );
				}
				button.connect( this, {
					'click': [ 'onActionButtonClick', button ],
					'resize': [ 'onActionButtonResize', button ]
				} );
...
OO.ui.ActionDialog.prototype.getTeardownProcess = function ( data ) {
	...
	return OO.ui.ActionDialog.super.prototype.getTeardownProcess.call( this, data )
		.next( function () {
			...
			for ( i = 0, len = this.actionButtons.length; i < len; i++ ) {
				button = this.actionButtons[i];
...
OO.ui.ActionDialog.prototype.executeAction = function ( action ) {
	...
	return this.getActionProcess( action ).execute()
		...
		.done( OO.ui.bind( this.close, this, { 'action': action } ) );
};
...
OO.ui.MessageDialog.prototype.getSetupProcess = function ( data ) {
	...
	return OO.ui.MessageDialog.super.prototype.getSetupProcess.call( this, data )
		.next( function () {
			...
			if ( this.safeActionButton ) {
				this.$actions.append( this.safeActionButton.$element );
				this.safeActionButton.toggleFrame( false );
			}
			if ( this.otherActionButtons.length ) {
				for ( i = 0, len = this.otherActionButtons.length; i < len; i++ ) {
					this.$actions.append( this.otherActionButtons[i].$element );
					this.otherActionButtons[i].toggleFrame( false );
				}
			}
			if ( this.primaryActionButton ) {
				this.$actions.append( this.primaryActionButton.$element );
				this.primaryActionButton.toggleFrame( false );
			}
...
OO.ui.MessageDialog.prototype.fitActionButtons = function () {
	var i, len, item,
		useColumns = OO.ui.bind( function ( value ) {
			this.$actions
				.toggleClass( 'oo-ui-messageDialog-actions-rows', !value )
				.toggleClass( 'oo-ui-messageDialog-actions-columns', value );
		}, this );

	useColumns( true );
	// Detect clipping
	for ( i = 0, len = this.actionButtons.length; i < len; i++ ) {
		item = this.actionButtons[i];
...
OO.ui.ProcessDialog.prototype.onActionButtonResize = function ( button ) {
	if ( button === this.safeActionButton || button === this.primaryActionButton ) {
		this.fitLabel();
	}
	return OO.ui.ProcessDialog.super.prototype.onActionButtonResize.call( this, button );
};
...
OO.ui.ProcessDialog.prototype.getSetupProcess = function ( data ) {
	return OO.ui.ProcessDialog.super.prototype.getSetupProcess.call( this, data )
		.next( function () {
			...
			// Attach action buttons
			if ( this.primaryActionButton ) {
				this.$primaryActions.append( this.primaryActionButton.$element );
				this.primaryActionButton.toggleFrame( false );
			}
			if ( this.otherActionButtons.length ) {
				for ( i = 0, len = this.otherActionButtons.length; i < len; i++ ) {
					this.$otherActions.append( this.otherActionButtons[i].$element );
					this.otherActionButtons[i].toggleFrame( true );
				}
				this.$body.css( 'bottom', this.$foot.outerHeight( true ) );
			}
			if ( this.safeActionButton ) {
				this.$safeActions.append( this.safeActionButton.$element );
				this.safeActionButton.toggleFrame( false );
			}
		}, this );
};
