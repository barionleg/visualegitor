...
OO.ui.ButtonElement.prototype.setButtonElement = function ( $button ) {
	if ( this.$button ) {
		this.$button
			...
			.remveAttr( 'role accesskey tabindex' )
...
OO.ui.ClippableElement.prototype.setClippableElement = function ( $clippable ) {
	if ( this.$clippable ) {
		...
		this.$clippable.css( { overflowX: '', width: '', overflowY: '', height: '' } );
	}
 ...
 *
 * A flag, when set, adds a CSS class on the `$element` by combing `oo-ui-flaggedElement-` with
...
OO.ui.FlaggedElement.prototype.setFlaggedElement = function ( $flagged ) {
	var classNames = this.flagClasses.join( ' ' );
...
OO.ui.FlaggedElement.prototype.clearFlags = function () {
	...
	for ( flag in this.flags ) {
		...
		while ( ( index = this.flagClasses.indexOf( className ) ) >= 0 ) {
			this.flagClasses.splice( index, 1 );
		}
	}
...
OO.ui.FlaggedElement.prototype.setFlags = function ( flags ) {
	var i, len, flag, index, className,
		changes = {},
		add = [],
		remove = [],
		classPrefix = 'oo-ui-flaggedElement-';
	...
	if ( typeof flags === 'string' ) {
		...
		if ( !this.flags[flags] ) {
			...
			add.push( className );
			this.flagClasses.push( className );
		}
 ...
 *  language
 * @cfg {string} [iconTitle] Icon title text or a function that return text
 */
 ...
 * @static
 * @inheritable
 * @property {string|Function|null} Icon title text, a function that return text or null for no
 *  icon title
 */
 ...
 * Set icon title.
 *
 * @param {string|Function|null} icon Icon title text, a function that return text or null
 *  for no icon title
 * @chainable
 */
