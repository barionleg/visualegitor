 ...
 *
 * Windows are sized using symbolc names which are interpreted by the window manager. If the
 * requested size is not recognized, the window manager will choose a sensible fallback.
 *
 * @constructor
 * @param {OO.ui.WindowManager} manager Manager of window
 * @param {Object} [config] Configuration options
 * @cfg {string} [size] Symbolic name of dialog size, `small`, `medium`, `large` or `full`; omit to
 *   use #static-defaultSize
 * @fires initialize
 */
OO.ui.Window = function OoUiWindow( manager, config ) {
	...
	this.opened = null;
	this.timing = null;
	this.size = config.size || this.constructor.static.defaultSize;
 ...
 * @property {string}
 */
OO.ui.Window.static.defaultSize = 'medium';
 ...
 * Get the height of the dialog contents.
 *
 * @return {number|false} Content height, false for max height
 */
...
OO.ui.Window.prototype.setup = function ( data ) {
	...
	this.frame.load().done( OO.ui.bind( function () {
		...
		this.getSetupProcess( data ).execute().done( OO.ui.bind( function () {
			...
			this.$element.addClass( 'oo-ui-window-setup' ).width();
			this.frame.$content.addClass( 'oo-ui-window-content-setup' ).width();
			deferred.resolve();
		}, this ) );
	}, this ) );
...
OO.ui.Window.prototype.ready = function ( data ) {
	...
	this.getReadyProcess( data ).execute().done( OO.ui.bind( function () {
		...
		this.frame.$content.addClass( 'oo-ui-window-content-ready' ).width();
		deferred.resolve();
	}, this ) );
...
OO.ui.Window.prototype.hold = function ( data ) {
	...
	this.getHoldProcess( data ).execute().done( OO.ui.bind( function () {
		this.frame.$content.find( ':focus' ).blur();
		// Force redraw by asking the browser to measure the elements' widths
		this.$element.removeClass( 'oo-ui-window-ready' ).width();
		this.frame.$content.removeClass( 'oo-ui-window-content-ready' ).width();
		deferred.resolve();
	}, this ) );
...
OO.ui.Window.prototype.teardown = function ( data ) {
	...
	this.getTeardownProcess( data ).execute().done( OO.ui.bind( function () {
		// Force redraw by asking the browser to measure the elements' widths
		this.$element.removeClass( 'oo-ui-window-setup' ).width();
		this.frame.$content.removeClass( 'oo-ui-window-content-setup' ).width();
		this.$element.hide();
		this.visible = false;
		deferred.resolve();
	}, this ) );
 ...
 *
 * - {@link #openWindow} method is used to start opening
 * - {@link #event-opening} event is emitted
 * - {@link OO.ui.Window#getSetupProcess} method is called on the window and its result executed
 * - {@link #event-setup} event is emitted
 * - {@link OO.ui.Window#getReadyProcess} method is called on the window and its result executed
 * - {@link #event-ready} event is emitted
 * - {@link #event-opened} event is emitted
 * - {@link #closeWindow} method is used to start closing
 * - {@link #event-closing} event is emitted
 * - {@link OO.ui.Window#getHoldProcess} method is called on the window and its result executed
 * - {@link #event-hold} event is emitted
 * - {@link OO.ui.Window#getTeardownProcess} method is called on the window and its result executed
 * - {@link #event-teardown} event is emitted
 * - {@link #event-closed} event is emitted
 *
 * Each process (setup, ready, hold and teardown) can be extended in subclasses. Each process is
 * executed in series, so asynchonous processing can complete. Always assume window processes have
 * asynchronous behavior to ensure your use of windows is future proof. See OO.ui.Process for more
 * details about how to work with processes. Some events, as well as the #open and #close methods,
 * provide promises which are resolved when the window enters a new state.
 *
 * @constructor
 * @param {Object} [config] Configuration options
 * @cfg {OO.ui.Factory} [factory] Window factory to use for automatic instantiation
 ...
 * Window is opened.
 *
 * Fired when the window is completely opened.
 *
 * @event opened
 * @param {OO.ui.Window} win Window that's been opened
 * @param {jQuery.Promise} closing Promise resolved when window begins closing closed; when the
 *   promise is resolved the first argument will be a promise which will be resolved when the window
 *   is closed, the second argument will be the closing data
 * @param {Object} data Window opening data
 */
 ...
 *   is resolved the first argument will be a the closing data
 * @param {Object} data Window closing data
 */
 ...
 * Window is held.
 *
 * Fired after the hold process has been executed.
 *
 * @event hold
 * @param {OO.ui.Window} win Window that's been held
 * @param {Object} data Window closing data
 */
 ...
 *
 * @event teardown
 * @param {OO.ui.Window} win Window that's being torn down
 * @param {Object} data Window closing data
 */
 ...
 * Window is closed.
 *
 * Fired after the window is completely closed.
 *
 * @event closed
...
OO.ui.WindowManager.prototype.afterWindowResize = function () {
	if ( this.current ) {
		this.updateWindowSize( this.current );
	}
};
...
/**
 * Handle managed window resize events.
 *
 * @param {OO.ui.Window} win Window being resized
 * @param {string} size Symbolic size name, e.g. 'small', 'medium', 'large', 'full'
 */
OO.ui.WindowManager.prototype.onManagedWindowResize = function ( win ) {
	if ( win === this.currentWindow ) {
		this.updateWindowSize( win );
	}
};
 ...
 * @return {OO.ui.Window} Managed window
 * @throws {Error} If the symbolic name is unrecognized by the factory
 * @throws {Error} If the symbolc name unrecognized as a managed window
 */
OO.ui.WindowManager.prototype.getWindow = function ( name ) {
	...
	if ( !( win instanceof OO.ui.Window ) ) {
		if ( this.factory ) {
			...
			throw new Error(
				'Cannot get unmanaged window: symbolc name unrecognized as a managed window'
			);
		}
	}
 ...
 * @param {Object} [data] Window opening data
 * @return {jQuery.Promise} Promise resolved when window is done opening
 */
OO.ui.WindowManager.prototype.openWindow = function ( win, data ) {
	...
	if ( typeof win === 'string' ) {
		...
		throw new Error( 'Cannot open window: window is not attached to manager' );
	}

	if ( this.opening || this.opened ) {
		// If a window is currently opening or opened, close it first
		preparing = this.closeWindow( this.currentWindow );
	} else if ( this.closing ) {
		// If a window is currently closing, wait for it to complete
		preparing = this.closing;
	} else {
		// Otherwise proceed immediately
		preparing = $.Deferred().resolve().promise();
	}

	preparing.done( OO.ui.bind( function () {
		if ( this.modal ) {
			// Prevent scrolling in top-level window
			this.$( window ).on( 'mousewheel', this.onWindowMouseWheelHandler );
			this.$( document ).on( 'keydown', this.onDocumentKeyDownHandler );
			// Respond to changes in top-level window dimensions
			this.$( window ).on(
		...
		this.emit( 'opening', win, opening, data );
		setTimeout( OO.ui.bind( function () {
			win.setup( data ).then( OO.ui.bind( function () {
				this.emit( 'setup', win, data );
				setTimeout( OO.ui.bind( function () {
					win.ready( data ).then( OO.ui.bind( function () {
						this.emit( 'ready', win, data );
						this.opening = null;
						this.opened = $.Deferred();
						opening.resolve( this.opened.promise(), data );
						this.emit( 'opened', win, this.opened.promise(), data );
					}, this ) );
				}, this ), this.getReadyDelay() );
			}, this ) );
		}, this ), this.getSetupDelay() );
	}, this ) );
...
OO.ui.WindowManager.prototype.closeWindow = function ( win, data ) {
	...
	if ( !win ) {
		throw new Error( 'Cannot close window: window is not attached to manager' );
	}
	if ( win !== this.currentWindow ) {
		throw new Error( 'Cannot close window: window already closed with different data' );
	}
	if ( this.closing ) {
		throw new Error( 'Cannot close window: window already closing with different data' );
	}

	if ( this.opening ) {
		// If the window is currently opening, close it when it's done
		preparing = this.opening;
	} else {
		// Otherwise proceed immediately
		preparing = $.Deferred().resolve().promise();
	}
	...
	preparing.done( OO.ui.bind( function () {
		...
		this.emit( 'closing', win, closing, data );
		this.opened = null;
		opened.resolve( closing, data );
		setTimeout( OO.ui.bind( function () {
			win.hold( data ).then( OO.ui.bind( function () {
				this.emit( 'hold', win, data );
				setTimeout( OO.ui.bind( function () {
					win.teardown( data ).then( OO.ui.bind( function () {
						this.emit( 'teardown', win, data );
						if ( this.modal ) {
						...
						this.emit( 'closed', win, data );
					}, this ) );
				}, this ), this.getTeardownDelay() );
			}, this ) );
		}, this ), this.getHoldDelay() );
	}, this ) );
...
OO.ui.WindowManager.prototype.addWindows = function ( windows ) {
	...
	for ( name in list ) {
		...
		win.connect( this, { 'resize': [ 'onManagedWindowResize', win ] } );
...
OO.ui.WindowManager.prototype.removeWindows = function ( names ) {
	...
	for ( i = 0, len = names.length; i < len; i++ ) {
		...
		promises.push( this.closeWindow( name ).then( OO.ui.bind( function () {
 ...
 * @constructor
 */
OO.ui.ActionButtonSet = function OoUiActionButtonSet() {
	// Mixin constructors
	OO.EventEmitter.call( this );
...
OO.mixinClass( OO.ui.ActionButtonSet, OO.EventEmitter );

/* Events */

/**
 * @event click
 * @param {OO.ui.ActionButtonWidget} button Button that was clicked
 */

/**
 * @event resize
 * @param {OO.ui.ActionButtonWidget} button Button that was resized
 */

/* Methods */

OO.ui.ActionButtonSet.prototype.getButtons = function () {
	return this.list.slice();
};

OO.ui.ActionButtonSet.prototype.getSafeButton = function () {
	return this.safe;
};

OO.ui.ActionButtonSet.prototype.getPrimaryButton = function () {
	return this.primary;
};

OO.ui.ActionButtonSet.prototype.getOtherButtons = function () {
	return this.others.slice();
};

OO.ui.ActionButtonSet.prototype.getButtonByAction = function ( action ) {
	return this.map[action];
};

OO.ui.ActionButtonSet.prototype.getButtonsByFlags = function ( flags ) {
	var i, len, flag,
		matches = [];

	for ( i = 0, len = flags.length; i < len; i++ ) {
		if ( $.isArray( this.flags[flags[i]] ) ) {
			matches.push.apply( matches, this.flags[flag] );
		}
	}
...
OO.ui.ActionButtonSet.prototype.addButtons = function ( buttons ) {
	var i, iLen, j, jLen, button, action, flag, flags;
	...
	for ( i = 0, iLen = buttons.length; i < iLen; i++ ) {
		...
		button.connect( this, {
			...
			'resize': [ 'emit', 'resize', button ]
		} );
		// Add to list
		this.list.push( button );
		// Add to flags
		flags = button.getFlags();
		if ( flags.length ) {
			for ( j = 0, jLen = flags.length; j < jLen; j++ ) {
				flag = flags[i];
				if ( !this.flags[flag] ) {
					this.flags[flag] = [];
				}
				this.flags[flag].push( button );
			}
		}
		// Add to safe/primary/others
		if ( !this.safe && button.hasFlag( 'safe' ) ) {
			this.safe = button;
		} else if ( !this.primary && button.hasFlag( 'primary' ) ) {
			this.primary = button;
		} else {
			this.others.push( button );
		}
		// Add to map
...
OO.ui.ActionButtonSet.prototype.removeButtons = function ( buttons ) {
	var i, iLen, index, flag, action, button;

	for ( i = 0, iLen = buttons.length; i < iLen; i++ ) {
		button = buttons[i];
		index = this.list.indexOf( button );
		if ( index !== -1 ) {
			// Disconnect events
			...
			// Remove from others
			index = this.others.indexOf( button );
			if ( index !== -1 ) {
				this.others.splice( index, 1 );
			}
			// Remove from flags
			for ( flag in this.flags ) {
				index = this.flags[flag].indexOf( button );
				if ( index !== -1 ) {
					this.flags[flag].splice( index, 1 );
				}
			...
			// Remove from map
			action = button.getAction();
			if ( action && this.map[action] === button ) {
				delete this.map[action];
			}
			// Reassign safe
			if ( this.safe === button ) {
				this.safe = this.flags.safe && this.flags.safe.length ?
					this.flags.safe[0] : null;
			}
			// Reassign primary
			if ( this.primary === button ) {
				this.primary = this.flags.primary && this.flags.primary.length ?
					this.flags.primary[0] : null;
			}
		}
	}
...
OO.ui.ActionButtonSet.prototype.clearButtons = function () {
	var i, len, button;
...
OO.ui.ActionDialog = function OoUiActionDialog( manager, config ) {
	...
	// Properties
	this.actionButtons = new OO.ui.ActionButtonSet();
	this.currentActionButton = null;
	this.pending = 0;

	// Events
	this.actionButtons.connect( this, {
		'click': 'onActionButtonClick',
		'resize': 'onActionButtonResize'
	} );
...
/**
 * List of OO.ui.ActionButtonWidget configuration options.
 *
 * @static
 * inheritable
 * @property {Object[]}
...
/**
 * Handle action button resized events.
 *
 * @param {OO.ui.ActionButtonWidget} button Button that was resized
 */
OO.ui.ActionDialog.prototype.onActionButtonResize = function () {
 ...
 * @param {OO.ui.ActionButtonWidget} button Button that was clicked
 */
OO.ui.ActionDialog.prototype.onActionButtonClick = function ( button ) {
	if ( !this.isPending() ) {
		this.currentActionButton = button;
		this.executeAction( button.getAction() );
	}
};
...
/**
 * Get set of action buttons.
 *
 * @return {OO.ui.ActionButtonSet}
 */
OO.ui.ActionDialog.prototype.getActionButtons = function () {
 ...
 *
 * @param {Object} [data] Dialog opening data
 * @param {Object[]} [data.actions] List of OO.ui.ActionButtonWidget configuration options for each
	...
	return OO.ui.ActionDialog.super.prototype.getSetupProcess.call( this, data )
		.next( function () {
			...
			for ( i = 0, len = actions.length; i < len; i++ ) {
				buttons.push(
					new OO.ui.ActionButtonWidget( $.extend( { '$': this.$ }, actions[i] ) )
				);
			}
			this.actionButtons.addButtons( buttons );
		}, this );
};
...
OO.ui.MessageDialog = function OoUiMessageDialog( manager, config ) {
	...
	// Properties
	this.verticalButtonLayout = null;
...
OO.ui.MessageDialog.static.defaultSize = 'small';
 ...
 * @inheritdoc
 */
OO.ui.MessageDialog.prototype.onActionButtonResize = function ( button ) {
	this.fitActionButtons();
	return OO.ui.ProcessDialog.super.prototype.onActionButtonResize.call( this, button );
};

/**
 * Toggle button layout between vertical and horizontal.
 *
 * @param {boolean} [value] Layout buttons vertically, omit to toggle
 * @chainable
 */
OO.ui.MessageDialog.prototype.toggleVerticalButtonLayout = function ( value ) {
	value = value === undefined ? !this.verticalButtonLayout : !!value;

	if ( value !== this.verticalButtonLayout ) {
		this.verticalButtonLayout = value;
		this.$actions
...
OO.ui.MessageDialog.prototype.getSetupProcess = function ( data ) {
	...
	return OO.ui.MessageDialog.super.prototype.getSetupProcess.call( this, data )
		.next( function () {
			var i, len, other,
				safe = this.actionButtons.getSafeButton(),
				primary = this.actionButtons.getPrimaryButton(),
				others = this.actionButtons.getOtherButtons();
			...
			if ( safe ) {
				this.$actions.append( safe.$element );
				safe.toggleFrame( false );
			}
			if ( others.length ) {
			...
			if ( primary ) {
				this.$actions.append( primary.$element );
				primary.toggleFrame( false );
			}
...
/**
 * Fit action buttons into columns or rows.
 *
 * Columns will be used if all labels can fit without overflow, otherwise rows will be used.
 */
OO.ui.MessageDialog.prototype.fitActionButtons = function () {
	var i, len, button,
		buttons = this.actionButtons.getButtons();

	// Detect clipping
	this.toggleVerticalButtonLayout( false );
	for ( i = 0, len = buttons.length; i < len; i++ ) {
		button = buttons[i];
		if ( button.$element.innerWidth() < button.$label.outerWidth( true ) ) {
			this.toggleVerticalButtonLayout( true );
			break;
		}
	}
};
 ...
 */
OO.ui.ProcessDialog.prototype.onRetryActionButtonClick = function () {
	this.hideErrors();
	this.executeAction( this.currentActionButton.getAction() );
};

/**
 * @inheritdoc
 */
OO.ui.ProcessDialog.prototype.onActionButtonResize = function ( button ) {
	if (
		button === this.actionButtons.getSafeButton() ||
		button === this.actionButtons.getPrimaryButton
	) {
		this.fitLabel();
	}
	return OO.ui.ProcessDialog.super.prototype.onActionButtonResize.call( this, button );
 ...
 * @inheritdoc
 */
OO.ui.ProcessDialog.prototype.getSetupProcess = function ( data ) {
	return OO.ui.ProcessDialog.super.prototype.getSetupProcess.call( this, data )
		.next( function () {
			var i, len, other,
				primary = this.actionButtons.getPrimaryButton(),
				safe = this.actionButtons.getSafeButton(),
			...
			// Attach action buttons
			if ( primary ) {
				this.$primaryActions.append( primary.$element );
				primary.toggleFrame( false );
			}
			if ( others.length ) {
				for ( i = 0, len = others.length; i < len; i++ ) {
			...
			if ( safe ) {
				this.$safeActions.append( safe.$element );
				safe.toggleFrame( false );
			}
			this.$body.css( 'bottom', this.$foot.outerHeight( true ) );
		}, this );
...
OO.ui.ProcessDialog.prototype.initialize = function () {
	...
	this.retryButton.connect( this, { 'click': 'onRetryActionButtonClick' } );
...
OO.ui.ProcessDialog.prototype.fitLabel = function () {
	...
	this.$label.css( { 'padding-left': width, 'padding-right': width } );
...
OO.ui.ProcessDialog.prototype.showErrors = function ( errors ) {
	...
	if ( recoverable ) {
		this.retryButton.clearFlags().setFlags( this.currentActionButton.getFlags() );
	} else {
		this.currentActionButton.setDisabled( true );
	}
 ...
 */
OO.ui.ActionButtonWidget = function OoUiActionButtonWidget( config ) {
	// Config intialization
	config = $.extend( { 'framed': false }, config );

	// Parent constructor
	OO.ui.ActionButtonWidget.super.call( this, config );
	...
	// Initialization
	this.$element.addClass( 'oo-ui-actionButtonWidget' );
};

/* Setup */

OO.inheritClass( OO.ui.ActionButtonWidget, OO.ui.ButtonWidget );
 ...
 * @return {string}
 */
OO.ui.ActionButtonWidget.prototype.getAction = function () {
	return this.action;
};
 ...
 * @chainable
 */
OO.ui.ActionButtonWidget.prototype.propagateResize = function () {
	var width, height;
 ...
 * @inheritdoc
 */
OO.ui.ActionButtonWidget.prototype.setIcon = function () {
	// Mixin method
 ...
 * @inheritdoc
 */
OO.ui.ActionButtonWidget.prototype.setLabel = function () {
	// Mixin method
	OO.ui.LabeledElement.prototype.setLabel.apply( this, arguments );
	this.propagateResize();
 ...
 * @inheritdoc
 */
OO.ui.ActionButtonWidget.prototype.setFlags = function () {
	// Mixin method
 ...
 * @inheritdoc
 */
OO.ui.ActionButtonWidget.prototype.clearFlags = function () {
	// Mixin method
 ...
 * @chainable
 */
OO.ui.ActionButtonWidget.prototype.toggle = function () {
	// Parent method
	OO.ui.ActionButtonWidget.super.prototype.toggle.apply( this, arguments );
	this.propagateResize();
