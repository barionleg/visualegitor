/*!
 * UnicodeJS v13.0.3-dev
 * https://www.mediawiki.org/wiki/UnicodeJS
 *
 * Copyright 2013-2023 UnicodeJS Team and other contributors.
...
/*!
 * UnicodeJS sentencebreak class.
 *
 * @copyright 2013–2023 UnicodeJS team and others; see AUTHORS.txt
 * @license The MIT License (MIT); see LICENSE.txt
 */
...
( function () {
	 ...
	 * @class singleton
	 */
	unicodeJS.sentencebreak = {};

	/* Static properties */

	unicodeJS.S_CONTINUE = new RegExp( '^(?:' + unicodeJS.charRangeArrayRegexp(
		unicodeJS.sentencebreakproperties.SContinue
	) + ')' );

	/* Methods */

	/**
	 * Find and analyze sentence boundaries within text
	 * See https://unicode.org/reports/tr29/#Sentence_Boundaries
	 *
	 * Each sentence boundary has text that looks like: STerm|ATerm + Close* + Sp*
	 * STerm - Unambiguous sentence termination character, such as "?" (U+003F QUESTION MARK)
	 * ATerm - Ambiguous sentence termination character, (only U+002E FULL STOP and variants)
	 * Close - Close brackets, close quotes etc
	 * Sp - whitespace
	 * As a special case, the text end is a sentence boundary even if there is no STerm|ATerm
	 *
	 * @param {string} text Text to analyze
	 * @return {Object[]} Sentence boundaries
	 *  'startIndex' - the start of the boundary region
	 *  'endIndex' - the end of the boundary region
	 *  'ambiguous' - whether the sentence terminating punctuation is ambiguous
	 *  'content' - An array describing the content that forms the boundary
	 *  'content.i.type' - The text type, saTerm|close|sp
	 *  'content.i.value' - The text value ('' if absent)
	 */
	unicodeJS.sentencebreak.getBoundaries = function ( text ) {
		var i, iLen, match, boundary,
			matches = this.getMatches( text ),
			boundaries = [];
		for ( i = 0, iLen = matches.length; i < iLen; i++ ) {
			match = matches[ i ];
			boundary = this.getBoundary( text, match );
			if ( boundary !== null ) {
	...
	/**
	 * Quickly find possible sentence boundaries for further examination
	 *
	 * @param {string} text The text to examine
	 * @return {Object[]} Potential matches
	 * @return {Object[]} return.i Regex match with groups [ sTerm, aTerm, close, sp ]
	 */
	unicodeJS.sentencebreak.getMatches = function ( text ) {
		var sTerm, aTerm, close, sp, re, matches, match;

		// Need a separate regex instance for every call, to do iteration
		sTerm = unicodeJS.charRangeArrayRegexp(
			unicodeJS.sentencebreakproperties.STerm
		);
		aTerm = unicodeJS.charRangeArrayRegexp(
			unicodeJS.sentencebreakproperties.ATerm
		);
		close = unicodeJS.charRangeArrayRegexp(
			unicodeJS.sentencebreakproperties.Close
		);
		sp = unicodeJS.charRangeArrayRegexp(
			unicodeJS.sentencebreakproperties.Sp
		);
		re = new RegExp(
		...
		while ( true ) {
			...
			match = re.exec( text );
			// The regex cannot fail to match
			matches.push( match );
		}
		return matches;
	};
	...
	/**
	 * Examine a potential sentence boundaryExamine a potential sentence boundaryExamine a potential sentence boundaryExamine a potential sentence boundary
	 *
	 * @param {string} text The text to examine
	 * @return {string[][]} Potential matches in format [ sTerm, aTerm, close, sp ]
	 */
	unicodeJS.sentencebreak.getBoundary = function ( text, match ) {
		var tail, head, lastWord,
			...
			ambiguous = !!aTerm && endIndex < text.length;

		if ( ambiguous ) {
			tail = text.slice( endIndex, text.length );
			head = text.slice( 0, match.index );

			// Trailing non-final punctuation: not a sentence boundary
			if ( unicodeJS.S_CONTINUE.exec( tail ) ) {
				return null;
			}
