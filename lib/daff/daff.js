...
coopy_Alignment.prototype = {
	...
	,addToOrder: function(l,r,p) {
		if(p == null) p = -2;
		if(this.order_cache == null) this.order_cache = new coopy_Ordering();
...
coopy_CompareTable.prototype = {
	...
	,alignCore2: function(align,a,b) {
		if(align.meta == null) align.meta = new coopy_Alignment();
		...
		if(ids != null) {
			...
			while(_g4 < ha) {
				...
				align.link(j,jb);
				used.h[jb] = 1;
				if(!used_reverse.h.hasOwnProperty(j)) reverse_pending_ct--;
				used_reverse.h[j] = 1;
			}
		} else {
			var N = 5;
			...
			while(_g8 < top) {
				...
				while( $it0.hasNext() ) {
					...
					if(!used.h.hasOwnProperty(val)) {
						...
						align.link(j4,val);
						used.h[val] = 1;
						if(!used_reverse.h.hasOwnProperty(j4)) reverse_pending_ct--;
						used_reverse.h[j4] = 1;
					}
				}
				var _g24 = 0;
			...
			while(_g9 < 2) {
				...
				if(pending_ct > 0) {
					...
					while(_g15 < ha) {
						...
						if(ka != kb) continue;
						if(used.h.hasOwnProperty(xb)) continue;
						align.link(xa,xb);
						used.h[xb] = 1;
						used_reverse.h[xa] = 1;
						pending_ct--;
			...
			while(_g10 < 2) {
				...
				if(reverse_pending_ct > 0) {
					...
					while(_g16 < hb) {
						...
						if(ka1 != kb1) continue;
						if(used_reverse.h.hasOwnProperty(xa1)) continue;
						align.link(xa1,xb1);
						used.h[xb1] = 1;
		...
		while(_g17 < ha) {
			var i2 = _g17++;
			if(!used_reverse.h.hasOwnProperty(i2)) align.link(i2,-1);
		}
		var _g18 = 1;
		while(_g18 < hb) {
			var i3 = _g18++;
			if(!used.h.hasOwnProperty(i3)) align.link(-1,i3);
		}
...
coopy_Coopy.diffAsHtml = function(local,remote,flags) {
	...
	if(o == null) o = new coopy_SimpleTable(0,0);
...
coopy_Coopy.diffAsAnsi = function(local,remote,flags) {
	...
	tool.cache_txt = "";
	if(flags == null) flags = new coopy_CompareFlags();
...
coopy_Coopy.prototype = {
	...
	,encodeTable: function(name,t,render) {
		...
		if(render == null) {
			...
			} else if(this.format_preference == "ndjson") txt = new coopy_Ndjson(t).render(); else if(this.format_preference == "html" || this.format_preference == "www") {
				this.renderTable(name,t);
				return null;
			} else if(this.format_preference == "sqlite") {
				this.io.writeStderr("! Cannot yet output to sqlite, aborting\n");
				return "";
			} else txt = haxe_Json.stringify(coopy_Coopy.jsonify(t),null,"  ");
	...
	,saveTables: function(name,os,use_color,is_diff) {
		...
		this.checkFormat(name);
		var render = null;
		if(use_color) render = new coopy_TerminalDiffRender(this.flags,this.delim_preference,is_diff);
	...
	,loadTable: function(name) {
		...
		if(ext == "json" || ext == "") try {
			...
			if(t1 == null) throw new js__$Boot_HaxeError("JSON failed");
			return t1;
		} catch( e ) {
			if (e instanceof js__$Boot_HaxeError) e = e.val;
			if(ext == "json") throw new js__$Boot_HaxeError(e);
		}
		this.format_preference = "csv";
		...
		while(more) {
			...
			while(_g1 < _g) {
				...
				if(tag == "--output") {
					...
					more = true;
					if(this.flags.acts == null) this.flags.acts = new haxe_ds_StringMap();
					{
	...
	,coopyhx: function(io) {
		var args = io.args();
		if(args[0] == "--keep") return coopy_Coopy.keepAround();
...
var coopy_HighlightPatch = $hx_exports.coopy.HighlightPatch = function(source,patch,flags) {
	...
	this.patch = patch;
	this.flags = flags;
	if(flags == null) this.flags = new coopy_CompareFlags();
...
coopy_HighlightPatch.prototype = {
	...
	,applyMetaRow: function(code) {
		...
		if(this.prev_meta == null) this.prev_meta = new haxe_ds_StringMap();
		if(this.next_meta == null) this.next_meta = new haxe_ds_StringMap();
	...
	,applyHeader: function() {
		...
		while(_g1 < _g) {
			...
			if(move) {
				if(this.headerMove == null) this.headerMove = new haxe_ds_StringMap();
	...
	,applyActionExternal: function(code) {
		...
		rc.action = code;
		this.checkAct();
		if(code != "+++") rc.cond = new haxe_ds_StringMap();
		if(code != "---") rc.val = new haxe_ds_StringMap();
		...
		while(_g1 < _g) {
			...
			if(cact == "+++" && code != "---") {
				if(txt != null && txt != "") {
					if(rc.val == null) rc.val = new haxe_ds_StringMap();
					rc.val.set(name,txt);
					have_column = true;
				}
			}
	...
	,checkAct: function() {
		...
		if(this.rowInfo.value != act) coopy_DiffRender.examineCell(0,0,this.view,act,"",act,"",this.rowInfo);
	}
	...
	,computeOrdering: function(mods,permutation,permutationRev,dim) {
		...
		if(ct > 0) {
			...
			while(_g2 < dim) {
				var i1 = _g2++;
				if(logical != null && meta_from_unit.h.hasOwnProperty(logical)) cursor = meta_from_unit.h[logical]; else cursor = null;
				if(cursor == null) {
					var v4 = starts.shift();
					cursor = v4;
					logical = v4;
...
coopy_Mover.move = function(isrc,idest) {
	...
	while(_g2 < len) {
		var i2 = _g2++;
		v = isrc[i2];
		if(in_dest.h.hasOwnProperty(v)) src.push(v);
	}
	var _g3 = 0;
	while(_g3 < len2) {
		var i3 = _g3++;
		v = idest[i3];
		if(in_src.h.hasOwnProperty(v)) dest.push(v);
	}
...
coopy_Ndjson.prototype = {
	...
	,addRow: function(r,txt) {
		var json = new haxe_format_JsonParser(txt).parseRec();
		if(this.columns == null) this.columns = new haxe_ds_StringMap();
...
coopy_SqlCompare.prototype = {
	...
	,validateSchema: function() {
		...
		if(access_error) throw new js__$Boot_HaxeError("Error accessing SQL table");
		if(pk_missing) throw new js__$Boot_HaxeError("sql diff not possible when primary key not available");
		var pk_change = false;
		...
		if(pk_change) throw new js__$Boot_HaxeError("sql diff not possible when primary key changes");
		return true;
	}
	,denull: function(x) {
		if(x == null) return -1;
	...
	,apply: function() {
		...
		if(this.align == null) this.align = new coopy_Alignment();
...
coopy_TableDiff.prototype = {
	...
	,hiliteSingle: function(output) {
		...
		if(this.builder == null) {
			if(this.flags.allow_nested_cells) this.builder = new coopy_NestedCellBuilder(); else this.builder = new coopy_FlatCellBuilder(this.flags);
		}
		output.resize(0,0);
		output.clear();
		this.reset();
...
var coopy_TableStream = function(t) {
	...
	if(this.h < 0) {
		var meta = t.getMeta();
		if(meta == null) throw new js__$Boot_HaxeError("Cannot get meta information for table");
		this.src = meta.getRowStream();
		if(this.src == null) throw new js__$Boot_HaxeError("Cannot iterate table");
	}
};
	...
	,render: function(t) {
		...
		while(_g < h) {
			...
			while(_g1 < w) {
				...
				if(x > 0) txt += this.codes.get("minor") + this.delim + this.codes.get("done");
...
coopy_Unit.prototype = {
	...
	,toString: function() {
		if(this.p >= -1) return coopy_Unit.describe(this.p) + "|" + coopy_Unit.describe(this.l) + ":" + coopy_Unit.describe(this.r);
...
CellView.prototype.equals = function(d1,d2) {
    ...
    var max_length = Math.max(d1.length, d2.length);
   	for (var i = 0; i < max_length; i++) {
        if (!(ve.dm.ElementLinearData.static.compareElementsUnannotated(d1[i], d2[i]))) {
            return false;
        }
    }
    return true;
