...
ve.ui.DesktopContext.prototype.updateDimensions = function ( transition ) {
	...
	if ( focusedNode ) {
		...
		$node = focusedNode.$shields.first();
		if ( this.embedded ) {
...
ve.ui.DesktopContext.prototype.show = function ( transition, repositionOnly ) {
	...
	if ( !this.showing && !this.hiding ) {
		...
		if ( inspector ) {
			...
			this.embedded = (
				focusedNode &&
				focusedNode.$shields.first().outerHeight() > this.$menu.outerHeight() * 4 &&
				focusedNode.$shields.first().outerWidth() > this.$menu.outerWidth() * 2
			);
