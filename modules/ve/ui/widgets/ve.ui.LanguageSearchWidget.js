...
ve.ui.LanguageSearchWidget = function VeUiLanguageSearchWidget( config ) {
	...
	var i, l, langCode,
		languageCodes = Object.keys( this.languages ).sort( $.uls.data.sortByAutonym );

	for ( i = 0, l = languageCodes.length; i < l; i++ ) {
		langCode = languageCodes[i];
		this.languageResultWidgets.push(
			new ve.ui.LanguageResultWidget(
				{
					'code': langCode,
					// In the default implementation, name === autonym, but subclasses
					// may override this.languages to provide localised language names
...
ve.ui.LanguageSearchWidget.prototype.addResults = function () {
	var i, iLen, j, jLen, languageResult, data, matchedProperty,
		matchProperties = ['autonym', 'name', 'code'],
