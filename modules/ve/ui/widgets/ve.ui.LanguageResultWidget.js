...
ve.ui.LanguageResultWidget = function VeUiLanguageResultWidget( data, config ) {
	...
	this.$autonym = this.$( '<div>' ).addClass( 've-ui-languageResultWidget-autonym' );
	this.$otherMatch = this.$( '<div>' ).addClass( 've-ui-languageResultWidget-otherMatch' );
	this.setLabel( this.$otherMatch.add( this.$autonym ) );
};
...
ve.ui.LanguageResultWidget.prototype.updateLabel = function ( query, matchedProperty ) {
	...
	// Reset text
	this.$autonym.text( data.autonym );
	...
	if ( matchedProperty ) {
		...
		if ( matchedProperty === 'autonym' ) {
			this.$autonym.html( $highlighted );
		} else {
			this.$otherMatch.html( $highlighted );
		}
	}
