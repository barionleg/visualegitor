...
ve.ui.MediaSizeWidget.prototype.validateDimensions = function () {
	...
	if (
		...
		Number( width ) <= 0 ||
		Number( height ) <= 0 ||
		// Check if the size exceeds original dimensions,
		// but only if the originalDimensions are set
		$.isNumeric( this.originalDimensions.width ) &&
		Number( width ) > this.originalDimensions.width ||
		$.isNumeric( this.originalDimensions.height ) &&
		Number( height ) > this.originalDimensions.height
	) {
		this.valid = false;
