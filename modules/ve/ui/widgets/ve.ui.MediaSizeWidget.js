...
ve.ui.MediaSizeWidget.prototype.setDimensions = function ( dimensions ) {
	...
	} else if ( this.aspectRatio !== null && dimensions.height && dimensions.height !== this.height && !dimensions.width ) {
...
ve.ui.MediaSizeWidget.prototype.validateDimensions = function () {
	...
	var width = this.widthInput.getValue(),
	...
	if (
		...
		Number( width ) <= 0 ||
		Number( height ) <= 0 ||
		// Check if the size exceeds original dimensions, but only if these are set
		$.isNumeric( this.originalDimensions.width ) && Number( width ) > this.originalDimensions.width ||
		$.isNumeric( this.originalDimensions.height ) && Number( height ) > this.originalDimensions.height
	) {
		this.valid = false;
...
ve.ui.MediaSizeWidget.prototype.onChangeWidth = function () {
	if ( Number( this.widthInput.getValue() ) !== Math.round( this.width ) && $.isNumeric( this.widthInput.getValue() ) ) {
...
ve.ui.MediaSizeWidget.prototype.onChangeHeight = function () {
	if ( Number( this.heightInput.getValue() ) !== Math.round( this.height ) && $.isNumeric( this.heightInput.getValue() ) ) {
