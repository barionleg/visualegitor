...
ve.ui.LinkInspector.prototype.onTargetInputChange = function () {
	var buttons,
		href = this.targetInput.getHref(),
		valid = this.targetInput.isValid();

	this.actions.forEach( { 'actions': 'open' }, function ( button ) {
		button.setHref( href ).setTarget( '_blank' ).setDisabled( !valid );
		// HACK: Chrome renders a dark outline around the button when it's a link, but causing it to
		// re-render makes it magically go away; this is incredibly evil and needs further investigation
		button.$element.hide().fadeIn( 0 );
	} );
};
