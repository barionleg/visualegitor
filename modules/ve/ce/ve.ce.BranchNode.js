...
ve.ce.BranchNode.prototype.setupSlugs = function () {
	...
	function canParentContainParagraph( node ) {
		if ( !node.parent ) {
			return false;
		}
		var childTypes = node.parent.getChildNodeTypes();
	...
	if ( this.$element.contents().length === 0 ) {
		...
		if ( canParentContainParagraph( this ) ) {
		...
		for ( i = 0, len = this.children.length; i < len; i++ ) {
			// Don't put slugs after internal nodes or in nodes which can't contain paragraphs
			if ( ve.dm.nodeFactory.isNodeInternal( this.children[i].model.type ) || !canParentContainParagraph( this.children[i] ) ) {
				continue;
			}
