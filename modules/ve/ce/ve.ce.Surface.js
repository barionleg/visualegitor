...
ve.ce.Surface.prototype.afterDocumentKeyDown = function ( e ) {
	var fixupCursor;
	ve.log( 'e=', e, ', this.cursorEvent=', this.cursorEvent );
...
ve.ce.Surface.prototype.checkUnicorns = function ( e, fixupCursor ) {
	...
	// oldRange can be uncollapsed, so consider end rather than start
	// XXX consider the logic in the case where !newRange.collapsed
	ancestor = ve.getCommonAncestor( oldRange.endContainer, newRange.endContainer );
	...
	oldPos = ve.getOffsetPath( ancestor, oldRange.endContainer, oldRange.endOffset );
	newPos = ve.getOffsetPath( ancestor, newRange.endContainer, newRange.endOffset );
	u1 = unicorns[ 0 ];
	u2 = unicorns[ 1 ];
...
ve.ce.Surface.prototype.onDocumentCompositionStart = function () {
	// Do nothing here; just poll on 'input'
};
...
/**
 * At a key down event, store the event and range
 *
 * @param {jQuery.Event} e key down event
 */
