...
$( function () {
	ve.init.platform.initialize().done( function () {
		...
		var $toolbar = $( '.ve-demo-targetToolbar' ),
			...
			// Menu widgets
			addSurfaceContainerButton = new OO.ui.ButtonWidget( { icon: 'add' } ),
 ...
 *
 * @param {string|ve.dm.Document} pageHtmlOrDoc HTML string or DM document
 */
ve.demo.SurfaceContainer.prototype.loadHtml = function ( pageHtmlOrDoc ) {
	var container = this;
	...
	this.surface = this.target.addSurface(
		pageHtmlOrDoc instanceof ve.dm.Document ? pageHtmlOrDoc :
			ve.dm.converter.getModelFromDom(
				ve.createDocumentFromHtml( pageHtmlOrDoc ),
...
ve.demo.SurfaceContainer.prototype.reload = function ( lang, dir ) {
	this.lang = lang;
	this.dir = dir;
	this.loadHtml( this.surface.getModel().getDocument() );
};
