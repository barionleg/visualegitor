...
module.exports = function ( grunt ) {
	...
	var modules = grunt.file.readJSON( 'build/modules.json' ),
		...
		desktopSABuildFiles = moduleUtils.makeBuildList( modules, [ 'visualEditor.desktop.standalone.build' ] ),
		mobileSABuildFiles = moduleUtils.makeBuildList( modules, [ 'visualEditor.mobile.standalone.build' ] );
	...
	grunt.initConfig( {
		...
		concat: {
			desktopSAjs: {
				dest: 'dist/visualEditor.desktop.standlone.js',
				src: introBuildFiles.scripts
					.concat( desktopSABuildFiles.scripts )
			},
			desktopSAcss: {
				dest: 'dist/visualEditor.desktop.standlone.css',
				src: introBuildFiles.styles
					.concat( desktopSABuildFiles.styles )
			},
			mobileSAjs: {
				dest: 'dist/visualEditor.mobile.standlone.js',
				src: introBuildFiles.scripts
					.concat( mobileSABuildFiles.scripts )
			},
			mobileSAcss: {
				dest: 'dist/visualEditor.mobile.standlone.css',
				src: introBuildFiles.styles
